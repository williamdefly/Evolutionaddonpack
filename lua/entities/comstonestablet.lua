...
ENT.PrintName = Lib.Language.GetMessage("entity_stone_tablet");
end
...
function ENT:Draw()
    ...
    if(LocalPlayer():GetEyeTrace().Entity == self.Entity && EyePos():Distance( self.Entity:GetPos() ) < 1024)then
		hook.Add("HUDPaint",tostring(self.Entity).."StonDev",function()
            ...
            draw.DrawText(Lib.Language.GetMessage("comstones_device"), "header", ScrW()/2+27, ScrH()/2-103, Color(0,255,255,255), 0)
            draw.DrawText(Lib.Language.GetMessage("comstones_channel")..chann, "center2", ScrW()/2+10, ScrH()/2-77, Color(209,238,238,255),0);
			draw.DrawText(Lib.Language.GetMessage("comstones_active")..act, "center2", ScrW()/2+10, ScrH()/2-57, Color(209,238,238,255),0);
...
function VGUI:Init()
	...
	DermaPanel:SetTitle(Lib.Language.GetMessage("comstones_device"))
    ...
    image:SetSize(10, 10);
    image:SetPos(10, 10);
    image:LoadTGAImage("materials/img/eap_logo", "MOD");

	local timlab = vgui.Create('DLabel')
	timlab:SetParent( DermaPanel )
	timlab:SetPos(20, 40)
	timlab:SetText(Lib.Language.GetMessage("comstones_opchannel"))
...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create("comstonestablet");
	ent:SetAngles(ang);
...
function ENT:StartTouch(ent)
	if (ent:GetClass()=="comstonesstone") then
...
function ENT:EndTouch(ent)
	if (ent:GetClass() == "comstonesstone" and table.HasValue(self.Stones, ent)) then
		self:EmitSound(self.Sounds.Place,100,math.random(98,102));
		ent.Tablet = NULL;
		local effectdata = EffectData()
			effectdata:SetEntity(ent)
...
function ENT:FindDevice()
	...
	for _,v in pairs(ents.FindByClass("comstonestablet")) do
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "comstonestablet", Lib.EAP_GmodDuplicator, "Data" )
end
