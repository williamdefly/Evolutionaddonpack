...
function ENT:Initialize()
	...
	self.ZPMMaxEnergy = Lib.CFG:Get("zpm_mk3","capacity",88000000);
end
...
language.Add("zpm_hub",Lib.Language.GetMessage("stool_atlantis_hub"));
end
...
function ENT:Draw()
	...
	if(not Lib.VisualsMisc("cl_draw_huds",true)) then hook.Remove("HUDPaint",tostring(self.Entity).."SGAH"); return end;
	local ang=EyeAngles()
    ang.y = ang.y;
    ...
    if(LocalPlayer():GetEyeTrace().Entity == self.Entity && EyePos():Distance( self.Entity:GetPos() ) < 1024)then
		hook.Add("HUDPaint",tostring(self.Entity).."SGAH",function()
    	    ...
    	    if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
		    	...
		    	draw.DrawText(Lib.Language.GetMessage("hud_capacities"), "zpmheader", ScrW() / 2 + 180 + w, ScrH() / 2 +45 - h, Color(209,238,238,255),0);
		    end
