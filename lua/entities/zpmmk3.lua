...
function ENT:Initialize()
	...
	self:AddResource("energy",Lib.CFG:Get("zpm_mk3","energy_capacity",1000000));
	self:SupplyResource("energy",Lib.CFG:Get("zpm_mk3","energy_capacity",1000000))
	self.MaxEnergy = Lib.CFG:Get("zpm_mk3","capacity",88000000);
	self.Energy = Lib.CFG:Get("zpm_mk3","capacity",88000000);
...
function ENT:Think()
	...
	if (self.Flow == 0) then
		...
		local entities = Lib.RD.GetEntListTable(self);
...
language.Add("zpmmk3",Lib.Language.GetMessage("stool_zpm_mk3"));
end
