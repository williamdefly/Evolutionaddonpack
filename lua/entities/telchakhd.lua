...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create("telchakhd");
...
hook.Add( "PlayerDeath", "Lib.Telchak", playerDies )
hook.Add( "PlayerSilentDeath", "Lib.Telchak", playerDies )
...
if (Lib and Lib.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "telchakhd", Lib.CAP_GmodDuplicator, "Data" )
end
...
ENT.PrintName = Lib.Language.GetMessage("entity_telchak");
end
...
function ENT:Draw()
	...
	if self.Healing then
		...
		if Lib.LOSVector(EyePos(), endpos, LocalPlayer(), 15) then
