		...
		if self.Rel < 5 then
			if Damage >= 250  then
				...
				elseif table.HasValue(self.DHDList, class) then
					if self.Scale > 30 then v:Remove();
					elseif(not v.GateSpawnerSpawned and not util.tobool(GetConVar("stargate_eap_dhd_protect"):GetInt()) or v.GateSpawnerSpawned and not util.tobool(GetConVar("stargate_eap_dhd_protect_spawner"):GetInt()))then
						v:DestroyEffect()
					end
...
function ENT:DamageShields()
	for k,v in pairs(Lib.FindEntInsideSphere(self.SplodePos, self.SplodeDist, "shield"))	do
		local dmgmul = 1/(self.SplodePos:Distance(v:GetPos())*50/self.Scale)^2
		v:Hit(self.Entity, self.SplodePos, 1e7*(20-self.Rel)*dmgmul)
	end

	for k,v in pairs(Lib.FindEntInsideSphere(self.SplodePos, self.SplodeDist, "shield_core_buble"))	do
		local dmgmul = 1/(self.SplodePos:Distance(v:GetPos())*50/self.Scale)^2
		v:Hit(self.Entity, self.SplodePos, 1e7*(20-self.Rel)*dmgmul)
	end
end
...
language.Add("gate_nuke", Lib.Language.GetMessage("gate_nuke"))
end
...
function ENT:NukeSunBeams() -- All Credits for this goes to Jinto :}) -- Moustache
	...
	if not Lib.VisualsWeapons("cl_gate_nuke_sunbeams") then return end
