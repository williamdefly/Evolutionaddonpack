	...
	if(
		...
		(
			...
			(
				...
				(e.Type ~= nil and not class:find("sg_"))
			)
		) or
...
function ENT:GetEntitiesForTeleport(e)
	if(self:Allowed(e)) then
		...
		local attached = Lib.GetConstrainedEnts(e); -- Based on Tad2020's faster method to catch attached entities
...
function ENT:TeleportEntity(t,base,basedata)
	...
	if(e:IsPlayer()) then
		...
		umsg.Start("Lib.CalcView.TeleportEffectStart",e);
		umsg.End();
	...
	-- ######### normal Entity teleport (The "rest")
		...
		e = Lib.Teleport:__Run(class,e,pos.New,ang.New,vel.New,pos.Old,ang.Old,vel.Old,ang.Delta);
