...
function ENT:Open()
    	...
    	elseif (class=="sg_universe") then
			self.OpeningDelay = 0.8;
			self.OpenTime = 2.2;
    	elseif (class=="sg_atlantis") then
			self.OpeningDelay = 0.9;
			self.OpenTime = 2.2;
    	elseif (class=="sg_movie" and not self:GetParent().Classic) then
			self.OpeningDelay = 0.8;
			self.OpenTime = 2.5;
    	else
        ...
        timer.Create("EventHorizonOpening"..self.Entity:EntIndex(),self.OpeningDelay,1,
                function()
                        if(IsValid(e)) then
								...
								if self.OpenEffect then
									util.Effect("event_horizon_stabilize",fx,true,true);
								end
					...
					fx:SetEntity(e);
					--##################### @ Llapp
					if(self.Entity:GetParent():GetClass()=="sg_orlin")then
						fx:SetRadius(2);
					...
					elseif(self.Entity:GetParent():GetClass()=="sg_supergate")then
						fx:SetRadius(3);
					    util.Effect("sg_kawoosh",fx,true,true);
					    -- This will kill people
					    local pos = self.Entity:GetPos();
					    ...
					    self:EHDissolve(pos+3*radius*normal,radius);
					    self:EHDissolve(pos+5*radius*normal,radius);
					elseif(self.Entity:GetParent():GetClass()=="sg_movie" and not self.Entity:GetParent().Classic)then
						...
						if (self.Entity:GetParent():GetClass()=="sg_universe") then
							fx:SetRadius(1);
						else
							fx:SetRadius(0);
						end
					    ...
					    self:EHDissolve(pos+5*radius*normal,radius);
                                ...
                                fx:SetOrigin(pos);
                                if self.OpenEffect then
                                	util.Effect("event_horizon_refract",fx,true,true);
                                end
                        end
                end
        );
...
function ENT:EHDissolve(pos,radius)
	...
	if (IsValid(self:GetParent()) and self:GetParent():GetClass()=="sg_supergate") then
		...
		e:Fire("kill","",4.4);
	elseif (IsValid(self:GetParent()) and self:GetParent():GetClass()=="sg_movie" and not self:GetParent().Classic) then
		for i=0,16 do
			e:Fire("hurt","",0.12*i);
		end
		e:Fire("kill","",2.0);
