...
ENT.Author = "Madman07, Boba Fett, AlexALX"
ENT.Instructions= ""
...
function ENT:Initialize()
	...
	self:CreateWireOutputs("End", "Close Lib","Minutes","Seconds");
...
util.AddNetworkString("EAP.DestinyTimer");

function ENT:Use(ply)
	if (self:GetWire("Disable Use")>=1) then return end

	net.Start("EAP.DestinyTimer");
...
net.Receive("EAP.DestinyTimer",function(len,ply)
	...
	elseif (type==1) then
		if (self.AutoStop) then
			...
			if (IsValid(gate) and not gate.IsOpen) then
				ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"destimer_error\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
				return
			end
		end
...
function ENT:FindGate(gt)
	...
	for _,v in pairs(ents.FindByClass("sg_*")) do
		if(v.IsLib and v:GetClass() != "sg_supergate" and (not IsValid(v.LockedDTimer) or v.LockedDTimer==self)) then
...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create("destinytimer");
	ent:SetAngles(ang);
		...
		if (self.MiliSeconds == -1) then
			...
			if (self.Seconds == -1 or self.CountUp and (self.Seconds==60 or time>=maxtime)) then
				...
				if (self.Minutes == -1 or self.CountUp and (self.Minutes==60 or time>=maxtime)) then
					...
					timer.Simple(0.1, function() if (IsValid(self)) then self:SetWire("Close Lib",0); end end);
...
ENT.PrintName = Lib.Language.GetMessage("entity_dest_timer");
end
...
net.Receive("EAP.DestinyTimer",function(len)
    ...
    image:SetImage("img/eap_logo");
 	...
 	title:SetText(Lib.Language.GetMessage("destimer_title"));
  	title:SetPos( 25, 0 );
 	title:SetSize( 400, 25 );

  	local title = vgui.Create( "DLabel", DermaPanel );
 	title:SetText(Lib.Language.GetMessage("destimer_time"));
  	title:SetPos( 10, 32 );
	...
	autostart:SetText(Lib.Language.GetMessage("destimer_autostart"))
	autostart:SizeToContents()
	autostart:SetPos(10, 55)
	autostart:SetValue( autocloseval )
	autostart:SizeToContents()
	autostart:SetTooltip(Lib.Language.GetMessage("destimer_autostart_desc"))
	...
	autostop:SetText(Lib.Language.GetMessage("destimer_autostop"))
	autostop:SizeToContents()
	autostop:SetPos(10, 75)
	autostop:SetValue( autocloseval )
	autostop:SizeToContents()
	autostop:SetTooltip(Lib.Language.GetMessage("destimer_autostop_desc"))
	...
	autoclose:SetText(Lib.Language.GetMessage("destimer_autoclose"))
	autoclose:SizeToContents()
	autoclose:SetPos(10, 95)
	autoclose:SetValue( autocloseval )
	autoclose:SizeToContents()
	autoclose:SetTooltip(Lib.Language.GetMessage("destimer_autoclose_desc"))
	...
	countup:SetText(Lib.Language.GetMessage("destimer_countup"))
	countup:SizeToContents()
	countup:SetPos(10, 115)
	countup:SetValue( autocloseval )
	countup:SizeToContents()
	countup:SetTooltip(Lib.Language.GetMessage("destimer_countup_desc"))
	...
	font:SetText(Lib.Language.GetMessage("destimer_font"))
	font:SizeToContents()
	font:SetPos(10, 135)
	font:SetValue( autocloseval )
	font:SizeToContents()
	font:SetTooltip(Lib.Language.GetMessage("destimer_font_desc"))
	...
	startButton:SetText(Lib.Language.GetMessage("destimer_start"))
	...
	stopButton:SetText(Lib.Language.GetMessage("destimer_stop"))
	...
	resetButton:SetText(Lib.Language.GetMessage("destimer_reset"))
	...
	closeButton:SetText(Lib.Language.GetMessage("destimer_close"))
