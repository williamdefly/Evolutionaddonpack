...
function ENT:Initialize()
	...
	self.EngageEnergy = Lib.CFG:Get("shield","engage_energy",500); -- This energy will be needed to engage the shield. You will get it back, when the shield collapses
	self.ConsumeMultiplier = Lib.CFG:Get("shield","consume_multiplier",1); -- As higher this is, as more energy it will take when enabled
	self.RestoreMultiplier = Lib.CFG:Get("shield","restore_multiplier",1); -- How fast can it restore it's health?
	self.StrengthConfigMultiplier = Lib.CFG:Get("shield","strength_multiplier",2); -- Doing this value higher will make the shiels stronger (look at the config)
	self.MaxSize = Lib.CFG:Get("shield","max_size",2048)
	self.Size = 80;
...
local function eap_shield_nuke(ent)
	if (not IsValid(ent)) then return end
	if (Lib.IsInShield(ent)) then return false end
end

hook.Add("Lib.GateNuke.DamageEnt","EAP.Shield.Nuke",eap_shield_nuke);
hook.Add("Lib.GateNuke.KillPlayer","EAP.Shield.Nuke",eap_shield_nuke);
hook.Add("Lib.SatBlast.DamageEnt","EAP.Shield.Nuke",eap_shield_nuke);
