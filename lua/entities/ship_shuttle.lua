...
function ENT:SpawnFunction(ply, tr) --######## Pretty useless unless we can spawn it @RononDex
	...
	local e = ents.Create("ship_shuttle")
...
function ENT:SpawnShieldGen()
	...
	if(IsValid(self)) then
		local e = ents.Create("shield_ship_generator")
		e:SetPos(self:GetPos())
...
ENT.PrintName = Lib.Language.GetMessage("ent_ship_shuttle");
end
ENT.RenderGroup = RENDERGROUP_BOTH
	...
	if((Shuttle)and(Shuttle:IsValid()and(Shuttle==self))) then
		for i=1,4 do
			if((p:KeyDown("EAP_KEYBOARD","FWD"))) then
				...
				if(Lib.VisualsShips("cl_shuttle_sprites")) then
