...
ENT.PrintName = Lib.Language.GetMessage("entity_black_hole");
end
...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("blackhole_power")
...
function ENT:Initialize()
	...
	self.MaxAmount = Lib.CFG:Get("black_hole","amount",500000);
	self.Resources = {};
	for _,v in pairs(Lib.CFG:Get("black_hole","resources",""):TrimExplode(",")) do
...
function ENT:StartTouch(ent)
	if IsValid(ent) then
		if (ent:GetClass() == "blackhole_power") then return end

		if(table.HasValue(self.Disallow,ent:GetClass())) then return end

		local allow = hook.Call("Lib.BlackHole.RemoveEnt",nil,ent,self);
	...
	for entKey,entVal in pairs(inRange) do
		if(not table.HasValue(self.Disallow,entVal:GetClass())) then
			...
			if(objRange < self.Range) then
				...
				local allow = hook.Call("Lib.BlackHole.PushEnt",nil,entVal,self);
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "blackhole_power", Lib.EAP_GmodDuplicator, "Data" )
end
