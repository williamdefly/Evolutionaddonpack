...
ENT.PrintName = Lib.Language.GetMessage("entity_asuran_weapon");
end
...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_asuran_beam_max"):GetInt()
	if(ply:GetCount("EAP_asuran_beam")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_asuran_weap\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = ang.y % 360;

	local ent = ents.Create("asuran_gateweapon");
	ent:SetAngles(ang);
	...
	ply:AddCount("EAP_asuran_beam", ent)
	return ent
end
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("EAP_asuran_beam", self.Entity)
	end
...
function ENT:FindTarget()
	for _, gate in pairs(ents.FindByClass("sg_*")) do
		if (gate:GetClass() == "sg_orlin") then return end
...
function ENT:StartFiring()
	...
	if(self.remoteGate.excessPowerLimit == nil) then
		self.remoteGate.excessPowerLimit = Lib.GetStargateEnergyCapacity(self.remoteGate) *
													  energyMultiplier
	end
...
function ENT:CreateBeam()
	...
	beam:Fire("TurnOn", 1)
	 self.Entity:SetNetworkedEntity("SmallBeam", beam);
		...
		util.Effect("Gate_Weapon_beams", inBeamInfo,true,true)

		local energyBeam = ents.Create("energetic_laser");
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "asuran_gateweapon", Lib.EAP_GmodDuplicator, "Data" )
end
