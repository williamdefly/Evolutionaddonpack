...
util.AddNetworkString("destinyconsole");
...
function ENT:Initialize()
	...
	self.Range = Lib.CFG:Get("mobile_dhd","range",3000);
...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_destcon_max"):GetInt()
	if(ply:GetCount("EAP_destcon")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_dest_console\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y+180) % 360;

	local ent = ents.Create("destinyconsole");
	ent:SetAngles(ang);
...
function ENT:Touch(ent)
	if not IsValid(self.LockedGate) then
		if (string.find(ent:GetClass(), "sg")) then
			local gate = Lib.FindGate(self, self.Range);
	...
	if (button and not self.Busy) then
		...
		elseif (button == 17) then
			...
			if self.HaveCore then
				local shield = Lib.FindShield(self.Entity);
		...
		elseif (button > 12 and button < 17) then
			...
			local number, KinoEnt = Lib.FindKino(ply);
...
net.Receive("destinyconsole",function(length, player)
...
function ENT:Think(ply)
	...
	local ply = Lib.FindPlayer(self.Entity:GetPos(), 400);
...
function ENT:OpenMenu(p)
	...
	if (IsValid(self.LockedGate)) then
		...
		e = Lib.FindGate(self.Entity, self.Range);
	end
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		self.Owner = ply;
		ply:AddCount("EAP_destcon", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "destinyconsole", Lib.EAP_GmodDuplicator, "Data" )
end