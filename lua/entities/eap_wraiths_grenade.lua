...
if SERVER then
	...
	function ENT:Initialize()
		...
		self.DetectRange = Lib.CFG:Get("eap_wraith_grenade","detect_range",200);
		self.BlastRange = Lib.CFG:Get("eap_wraith_grenade","blast_range",200);
		self.BlastDamage = Lib.CFG:Get("eap_wraith_grenade","blast_damage",300);
	...
	function ENT:Use(ply)
		if(self.Mine and IsValid(self.Owner) and ply == self.Owner) then
			if (!ply:HasWeapon("eap_eap_wraith_grenade")) then
				ply:Give("eap_eap_wraith_grenade");
				ply:GetWeapon("eap_wraith_grenade").Ammo = 1;
			else
				ply:GetWeapon("eap_wraith_grenade").Ammo = ply:GetWeapon("eap_wraith_grenade").Ammo+1;
			end			
			self:Remove();
		end
	end
