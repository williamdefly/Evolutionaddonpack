...
function ENT:Setup(startent, effect, up)
	...
	if (effect == "GateWep") then
		...
		util.Effect("Gate_Weapon_Out",fx, true, true);
	elseif (effect == "ONeill") then
		local fx = EffectData();
			fx:SetOrigin(startent:GetPos());
			fx:SetEntity(self.Entity);
		util.Effect("rg_ONeillBeam",fx, true, true);
	end
end
...
function ENT:Think()
	...
	self.StargateTrace = Lib.Trace:New(startpos,endpos,{self.Entity, self.Owner, self.Parent});
...
language.Add("energetic_laser",Lib.Language.GetMessage("energy_laser_kill"));
end
