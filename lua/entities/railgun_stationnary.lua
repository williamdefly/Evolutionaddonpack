...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_statrail_max"):GetInt()
	if(ply:GetCount("EAP_statrail")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_stat_rail\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = ang.y % 360

	local ent = ents.Create("railgun_stationnary");
...
function ENT:Shoot()
	...
	self.StargateTrace = Lib.Trace:New(self.ShootPos,self.ShootPos-self.Cann:GetForward() * 10^14);
	...
	local damage = Lib.CFG:Get("railgun_stationnary","damage",10);
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		ply:AddCount("EAP_statrail", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "railgun_stationnary", Lib.EAP_GmodDuplicator, "Data" )
end
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	...
	ENT.PrintName = Lib.Language.GetMessage("entity_stat_railgun");
end
ENT.RenderGroup = RENDERGROUP_BOTH;
...
function StatRailCalcView(Player, Origin, Angles, FieldOfView)
	local view = {}
	local p = LocalPlayer()
	local self = p:GetNetworkedEntity("ScriptedVehicle", NULL)

	if (self and self:IsValid() and self:GetClass()=="railgun_stationnary") then
...
hook.Add("CalcView", "StatRailCalcView", StatRailCalcView)
