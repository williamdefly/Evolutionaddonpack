...
function ENT:SpawnFunction(ply, tr) --######## Pretty useless unless we can spawn it @RononDex
	...
	local e = ents.Create("eap_oneill")
...
function ENT:FireBlast(diff)
	if(self.CanShoot) then
		...
		e:PrepareBullet(self:GetForward(), 10, 16000, 12, {self.Entity});
...
if (StarGate and StarGate.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "eap_oneill", StarGate.CAP_GmodDuplicator, "Data" )
end
...
--########## Keybinder stuff
local KBD = StarGate.KeyBoard:New("Oneill")
--Navigation
