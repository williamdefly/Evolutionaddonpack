...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_ori_beam_max"):GetInt()
	if(ply:GetCount("EAP_ori_beam")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_ori_beam\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	ent = ents.Create("oribeam_cannon");
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		ply:AddCount("EAP_ori_beam", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "oribeam_cannon", Lib.EAP_GmodDuplicator, "Data" )
end
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	ENT.Category = Lib.Language.GetMessage("cat_weapons");
	ENT.PrintName = Lib.Language.GetMessage("entity_ori_beam");
end
