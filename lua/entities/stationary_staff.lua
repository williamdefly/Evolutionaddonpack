...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_staffstat_max"):GetInt()
	if(ply:GetCount("EAP_staffstat")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_staff_stat\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = ang.y % 360

	local ent = ents.Create("stationary_staff");
	...
	ply:AddCount("EAP_staffstat", ent)
	return ent
end
...
function ENT:SpawnRest(p)
	...
	local ent1 = ents.Create("stationary_staff_part");
	...
	local ent2 = ents.Create("stationary_staff_part");
		...
		if (self.Driver:KeyDown( IN_ATTACK2 ) and not self.Pressed) then
			...
			if (self.Open) then
				...
				local fx = EffectData();
					...
					fx:SetRadius(80);
				util.Effect("energymuzzle",fx,true);
...
function ENT:FireWeapon()
	...
	util.Effect("energymuzzle",fx,true,true);
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		ply:AddCount("EAP_staffstat", self.Entity)
	end
end

if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "stationary_staff", Lib.EAP_GmodDuplicator, "Data" )
end
