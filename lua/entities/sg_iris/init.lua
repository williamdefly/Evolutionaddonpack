...
function ENT:Initialize()
	...
	if(#Lib.GetConstrainedEnts(self.Entity,1) == 0) then
	...
	timer.Create("StarGateIrisCheck"..self:EntIndex(),10.0,0,function()
		...
		if (IsValid(self) and IsValid(self.GateLink) and constraint.HasConstraints(self)) then
			local entities = Lib.GetConstrainedEnts(self,2);
			if(entities) then
...
function ENT:RegisterModules()
	...
	for _,v in pairs(file.Find("entities/sg_iris/modules/*.lua","LUA")) do
		--if (v=="iris_infinity.lua")then continue end
		...
		include("entities/sg_iris/modules/"..v);
...
function ENT:FindGate()
	...
	for _,v in pairs(ents.FindByClass("sg_*")) do
		if(v.IsStargate and v:GetClass() != "sg_supergate") then
...
function ENT:Toggle(ignore_energy)
	...
	if(self.NextAction <= CurTime() or reactivate) then
		...
		if(self.IsActivated or deactivate) then
			...
			if (not ignore_energy and IsValid(self.GateLink) and self.GateLink.EAPGateSpawnerSpawned and not self:IsComputer()) then return end
			if(self.HasRD and self.Entity:GetModel() == "models/zup/stargate/sga_shield.mdl") then
				local gate = self:FindGate();
...
function ENT:TrueActivate(deactivate,wire)
	if(self.NextAction <= CurTime()) then
		...
		elseif (not self.IsActivated and not deactivate) then
			if (not ignore_energy and IsValid(self.GateLink) and self.GateLink.EAPGateSpawnerSpawned and not self:IsComputer()) then
...
function ENT:OnTakeDamage(  dmginfo )
	if (not IsValid(self.Entity) or self.Entity:GetModel() != "models/zup/stargate/sga_shield.mdl" or not self.IsActivated or dmginfo:GetAttacker():GetClass() == "point_hurt" or dmginfo:GetAttacker():GetClass() == "kawooshhurt") then return end
