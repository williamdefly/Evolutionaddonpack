...
function ENT:Initialize()
	...
	local model = self.Parent.Owner:GetClass()
	if (model == "ship_shuttle") then self.Entity:SetModel("models/Madman07/shields/destiny_shield.mdl");
	elseif (model == "ship_puddle_jumper") then self.IsJumper = true;
	else self.Entity:SetModel("models/zup/shields/200_shield.mdl"); end
	...
	--if(self.Parent.ImmuneOwner) then
		...
		if (self.Parent.Owner.Owner) then
			self.nocollide[self.Parent.Owner.Owner] = true;
		end*/
	--end
...
function ENT:StartTouch(e)
	...
	if(self.Parent.Containment and self.AllowContainment or self.IsJumper and e.JumperPart) then
...
function ENT:Reflect(e,do_not_draw_hit)
	...
	if(e.Touch) then e:Touch(self.Parent.Owner) end
	if(e.StartTouch) then e:StartTouch(self.Parent.Owner) end;
...
function ENT:HitEffect(e,pos,strength)
	...
	local function draw_effect(shield)
		if(IsValid(shield)) then
			...
			util.Effect("shield_hiting",fx,true,true);
...
function ENT:DrawBubbleEffect(turn_off,hit)
	...
	if(self.DrawBubble or (IsValid(self.Parent) and self.Parent.DrawBubble)) then
		...
		util.Effect("ship_shield_engaging",fx,true,true);
	end
end
