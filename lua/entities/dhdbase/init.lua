	...
	timer.Simple(delay,
		function()
			if(IsValid(e)) then
				...
				e.DialledAddress = {};
				e.Target = nil;
				timer.Remove("_Lib.DeactivateDHDTimer"..e:EntIndex());
...
function ENT:TriggerInput(k,v)
	...
	elseif (k == "Disable Iris Toggle" and self:GetClass()=="dhd_atl_city") then
		if (v>0) then
...
function ENT:Touch(ent)
	if not IsValid(self.LockedGate) then
		if (string.find(ent:GetClass(), "sg")) then
			local gate = self:FindGate()
...
function ENT:Use(p)
	...
	if (self.IsCityDHD and IsValid(p) and p:IsPlayer() and not self.Disabled) then -- small override for IRIS, to let it press even, if gate are activated
		...
		if (btn and btn == "IRIS" and not self.WireNoIris) then
			local iris = Lib.FindIris(self:FindGate());
...
function ENT:Think()
	...
	local candialg = GetConVar("stargate_eap_candial_groups_dhd"):GetInt()
	if (self:GetClass()=="dhd_atl_city") then candialg = 1; end
	if (candialg != self.Entity:GetNetworkedInt("CANDIAL_GROUP_DHD")) then
		self.Entity:SetNetworkedInt("CANDIAL_GROUP_DHD",candialg);
	end
	candialg = GetConVar("stargate_eap_candial_groups_menu"):GetInt()
	...
	candialg = GetConVar("stargate_eap_dhd_letters"):GetInt()
...
function ENT:AddChevron(btn, nosound, lightup, gate, city, fail)
	--if(table.getn(self.DialledAddress) < 10) then
		if(not table.HasValue(self.DialledAddress,btn)) then
			timer.Remove("_Lib.DeactivateDHDTimer"..self.Entity:EntIndex());
			timer.Create("_Lib.DeactivateDHDTimer"..self.Entity:EntIndex(),20,1,
				function()
			...
			if (self.Entity:GetClass()=="dhd_atl_city" and btn == "#") then
			...
			if((not fail or btn == "#" or table.getn(self.DialledAddress)==9 and not table.HasValue(self.DialledAddress,"DIAL")) and not nosound) then
				...
				elseif(self.Entity:GetClass()=="dhd_atl" or self.Entity:GetClass()=="dhd_atl_city") then
...
function ENT:ButtonMode(btn)
	...
	if self:GetClass()=="dhd_atl_city" and not self:GetNetworkedBool("HasEnergy",false) then return end
...
function ENT:PressButton(btn, nolightup, no_menu)
	...
	if (btn and btn == "*" and num == 0) then
		if (e:GetClass() == "sg_universe") then return end
		self:AddChevron(btn, nosound, false, e);
		-- Prepare gate
		local gates = {}
		for _,v in pairs(ents.FindByClass("sg_*")) do
			if(v.IsStargate and e != v and not v.GatePrivat and v:GetClass() != "sg_supegate" and v:GetClass() != "sg_orlin") then
	...
	if (btn and btn == "DIAL") then
		if(e:GetClass()=="sg_orlin" and e.IsOpen or IsValid(e.Target) and e.Target:GetClass()=="sg_orlin" and e.Target.IsOpen) then return end
		...
		if (num == 0) then
			...
			if (not no_menu and not self.DisMenu and GetConVar("stargate_eap_dhd_menu"):GetInt()>=1) then
				local candialg = GetConVar("stargate_eap_candial_groups_dhd"):GetBool();
				if (self.Entity:GetClass()=="dhd_atl_city") then candialg = true; end
	...
	if (btn and ((num < allowed_symbols or allowed_symbols==6 and num==6 and btn == "#") and (not table.HasValue(self.DialledAddress,"#") or btn == "#") and (btn != "#" or btn == "#" and (num >= 6 and num <= 8)) or btn == "DIAL") or table.HasValue(self.DialledAddress,btn) and (not table.HasValue(self.DialledAddress,"#") or btn == "#")) then
		...
		if (IsValid(e) and e:GetClass()=="sg_atlantis" and self.DisRingRotate) then atlantis = true end
		if (not table.HasValue(self.DialledAddress,"DIAL")) then
		    local city = false
			if (btn == "DIAL" and class == "dhd_atl_city") then city = true end
			if table.HasValue(self.DialledAddress,btn) then
				...
				if(btn == "#" or num==8 or btn == "DIAL") then
					...
					if (e:GetClass()=="sg_atlantis") then
		...
		if (not remove and IsValid(e) and (e:GetClass()=="sg_sg1" or e:GetClass()=="sg_infinity" or e:GetClass()=="sg_movie" or atlantis) and (btn=="#" or num==8 or atlantis) and (not self.DisRingRotate and GetConVar("stargate_eap_dhd_ring"):GetBool() or atlantis and self.DisRingRotate or e.Ring.WireMoving)) then
			if (atlantis) then
		...
		if(IsValid(e)) then
			if(btn == "DIAL") then
				...
				if(e.Dialling and e.Outbound and not e.IsOpen) then -- Only allow outbound gates to get disabled by DHD during a call
					...
					if(table.getn(self.DialledAddress) >= 8 and table.getn(self.DialledAddress) <= 10 and class != "dhd_atl_city") then
						e.DialledAddress = self.DialledAddress;
						...
						if (not no_menu and IsValid(self.LastPlayer)) then
							umsg.Start("Lib.DialMenuDHDClose",self.LastPlayer);
							umsg.End();
						end
						self.Target = e; -- Needs to be set, so the gate does not "relightupt" this DHD on dial
						self:SetBusy(2.5);
					elseif(table.getn(self.DialledAddress) >= 6 and table.getn(self.DialledAddress) <= 9 and class == "dhd_atl_city") then
...
concommand.Remove("_Lib.DHD.AddSymbol_Group"); -- In case of a lua_reloadents
concommand.Add("_Lib.DHD.AddSymbol_Group",
	function(_,_,arg)
...
function ENT:RampsDHD(t)
	...
	if (self.Entity:GetClass()=="dhd_con") then
		RampOffset = Lib.RampOffset.DHDC;
	end
