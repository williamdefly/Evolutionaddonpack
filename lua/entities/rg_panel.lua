...
function ENT:FindRing()
	...
	for _,v in pairs(ents.FindByClass("rg_base*")) do
...
function ENT:DoCallback(range, address)
	...
	if not self.RingBase.Busy then
		if (self.RingBase:GetClass() == "rg_base_ancient" and address == "3571") then
			local nearest_ring = self.RingBase:FindNearest("")
			if (nearest_ring == false) then return end
			if nearest_ring:GetClass() == "rg_base_ancient" then
...
function PANEL:Init()
	...
	self.Logo:SetSize(16,16);
 	...
 	self.L1:SetText(Lib.Language.GetMessage("ring_dial"))
 	self.L1:SetFont("OldDefaultSmall")

 	self.Button = vgui.Create( "RingDialButtonCap", self)
	self.Button:SetText(Lib.Language.GetMessage("ring_dialb"))
