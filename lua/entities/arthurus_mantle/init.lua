...
function ENT:Initialize()
	...
	local tbl = scripted_ents.Get("sg_base");
...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create("arthurus_mantle");
	ent:SetAngles(ang);
	...
	if IsValid(ply) then
		if (ply:GetNetworkedBool("ArthurCloaked", false)) then -- uncloack
			...
			local fx = EffectData();
				...
				fx:SetEntity(ply);
			util.Effect("arthurcloak",fx,true,true);

			local fx2 = EffectData();
				fx2:SetEntity(ply);
			util.Effect("arthurcloaklight",fx2,true,true);

			local fx3 = EffectData();
				fx3:SetEntity(self.Entity);
			util.Effect("arthurcloaklight",fx3,true,true);
...
hook.Add( "PlayerDeath", "Lib.Arthur", playerDies )
hook.Add( "PlayerSilentDeath", "Lib.Arthur", playerDies )
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "arthurus_mantle", Lib.EAP_GmodDuplicator, "Data" )
end
