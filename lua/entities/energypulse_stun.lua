...
ENT.NoAutoClose = true; -- Will not cause an autoclose event on the Libs!
...
function ENT:Initialize()
	...
	-- Config
	self.Radius = 20+30*self.Size;  --Lib.CFG:Get("staff","radius",50);
	self.Damage = 30+40*self.Size;  --Lib.CFG:Get("staff","damage",150);
	self.MaxPasses = 5; --Lib.CFG:Get("staff","maxpasses",5);
	self.Passes = 1;
...
function ENT:Touch(ent)
	if (ent:GetClass() == "shields" or ent:GetClass() == "shield_ship") then
		self:Destroy();
	end
end
...
function ENT:Stun( Ent )
	...
	if IsValid( Ent ) then
		...
		local allow = hook.Call("Lib.Player.Stun",nil,Ent,self.Owner);
...
Lib.Teleport:Add("energy_pulse_stun",ENT.FixAngles);
