...
function ENT:SpawnFunction( ply, tr )
	...
	if (IsValid(ply)) then
		local PropLimit = GetConVar("EAP_destmedium_max"):GetInt()
		if(ply:GetCount("EAP_destmedium")+1 > PropLimit) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_dest_med\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			return
		end
	end
	...
	local ent = ents.Create("turret_destmed");
	...
	if (IsValid(ply)) then
		ply:AddCount("EAP_destmedium", ent)
	end
	ent:SpawnRest(ply);
	ent.Duped = true;
	return ent
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		local PropLimit = GetConVar("EAP_destmedium_max"):GetInt()
		if(ply:GetCount("EAP_destmedium_max")+1 > PropLimit) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_dest_med\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			self.Entity:Remove();
			return
		end
		ply:AddCount("EAP_destmedium", self.Entity)
	end
	self.Duped = true;
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "turret_destmed", Lib.EAP_GmodDuplicator, "Data" )
end
...
function ENT:SpawnRest(p)
	...
	local ent = ents.Create("turret_part");
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	ENT.Category = Lib.Language.GetMessage("cat_weapons");
	ENT.PrintName = Lib.Language.GetMessage("entity_dest_med");
end
