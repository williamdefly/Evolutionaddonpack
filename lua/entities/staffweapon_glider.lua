...
function ENT:Initialize()
	...
	self.energy_drain = Lib.CFG:Get("staff_stationary","energy_per_shot",100);
	self.Delay = Lib.CFG:Get("staff_stationary","delay",0.3);
...
function ENT:SpawnFunction(p,t)
	...
	local e = ents.Create("staffweapon_glider");
...
function ENT:FireWeapon()
	...
	local pos = self.Entity:GetPos()+Lib.VelocityOffset({Velocity=vel,Direction=up,BoundingMax=self.Entity:OBBMaxs().z});
...
function ENT:Use(p)
	...
	local id = "Lib.StaffWeapon"..self.Entity:EntIndex();
	local e = self.Entity;
	timer.Destroy(id);
...
language.Add("staff_weapon_glider",Lib.Language.GetMessage("glider_staff")); -- Some idiot got smashed
end
-- Kill Icon
