...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_dakara_max"):GetInt()
	if(ply:GetCount("EAP_dakara")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_dakara\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y+180) % 360;

	local ent = ents.Create("dakarabuilding");
	ent:SetAngles(ang);
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("EAP_dakara", self.Entity)
	end
...
function ENT:SpawnStuff(p)
	...
	local ent = ents.Create("ancient_controlpanel");
	ent:SetAngles(ang);
...
function ENT:SpawnChargingEffect()
   ...
   timer.Simple(0.1, function()
      ...
      util.Effect("dakaracharging", effectInfo)
   end)
end
...
function ENT:CreateWave()
	...
	self.Wave = ents.Create("dakarawave")
...
function ENT:FindGate()
	...
	for _,v in pairs(ents.FindByClass("sg_*")) do
		if(v.IsStargate and v:GetClass() != "sg_supergate" and v:GetClass() != "sg_orlin" and v:GetClass() != "sg_universe" and not v.GateGalaxy) then
...
function ENT:FindAllGate()
	local gate = {};
	for _,v in pairs(ents.FindByClass("sg_*")) do
		if(v.IsStargate and v:GetClass() != "sg_supergate" and v:GetClass() != "sg_orlin" and v != self.DialGate and v:GetClass() != "sg_universe" and not v.GateGalaxy) then
			table.insert(gate,v);
			v.Target = v;
		end
	end
