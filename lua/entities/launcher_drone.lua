...
function ENT:Initialize()
	...
	self.energy_drain = Lib.CFG:Get("drone","energy_per_shot",200);
	self.reloadtime = Lib.CFG:Get("drone","delay",0.2);
	self.DroneMaxSpeed = Lib.CFG:Get("drone","maxspeed",6000);
	self.TrackTime = 1000000;
	self.Drones = {};
	self.MaxDrones = Lib.CFG:Get("drone","max_drones",8);
...
function ENT:SpawnFunction(ply,tr)
	...
	local PropLimit = GetConVar("EAP_launchdrone_max"):GetInt()
	if(ply:GetCount("EAP_launchdrone")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_drone_laun\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y+180) % 360;

	local ent = ents.Create("launcher_drone");
	ent:SetAngles(ang);
	...
	ply:AddCount("EAP_launchdrone", ent)
	return ent
end
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("EAP_launchdrone", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "launcher_drone", Lib.EAP_GmodDuplicator, "Data" )
end
...
function ENT:Think()
	...
	if(self.Launched) then
		...
		local offset = Lib.VelocityOffset({Velocity=vel,Direction=up,BoundingMax=self.Entity:OBBMaxs().z});
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	ENT.Category = Lib.Language.GetMessage("cat_weapons");
	ENT.PrintName = Lib.Language.GetMessage("entity_drone");
end
-- Kill Icon
if(file.Exists("materials/weapons/drone_killicon.vmt","GAME")) then
	killicon.Add("launcher_drone","weapons/drone_killicon",Color(255,255,255));
	killicon.Add("drones","weapons/drone_killicon",Color(255,255,255));
end
