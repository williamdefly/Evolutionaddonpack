...
function ENT:SpawnFunction( ply, tr)
	...
	local PropLimit = GetConVar("EAP_lantholo_max"):GetInt()
	if(ply:GetCount("EAP_lantholo")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_holo\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ent = ents.Create("lanteanholo");
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		self.Owner = ply;
		ply:AddCount("EAP_lantholo", self.Entity)
	end

end

if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "lanteanholo", Lib.EAP_GmodDuplicator, "Data" )
end
...
ENT.PrintName = Lib.Language.GetMessage("entity_lant_holo");
end

ENT.RenderGroup = RENDERGROUP_TRANSLUCENT

if (Lib==nil or Lib.MaterialFromVMT==nil) then return end
ENT.Stars = Lib.MaterialFromVMT(
	"Stars",
	[["Sprite"
	{
