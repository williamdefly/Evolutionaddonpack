...
function ENT:SpawnFunction(p,t)   --############ @  Llapp
	...
	local e = ents.Create("stargate_asuran_sat") ;
...
function ENT:AddGate(p)  --############ @  Llapp
	...
	local l = ents.Create("sg_atlantis");
	l:DrawShadow(false);
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "stargate_asuran_sat", Lib.EAP_GmodDuplicator, "Data" )
end
...
ENT.PrintName = Lib.Language.GetMessage("entity_asuran_satellite");
end
