...
function ENT:Initialize()
	...
	self.Fuel = Lib.CFG:Get("mini_drone","distance",10000);
	self.CurrentVelocity = 500;
	self.MaxVelocity = Lib.CFG:Get("mini_drone","maxspeed",6000)/6;
	self.Created = CurTime();
	self.Randomness = math.random(6,14)/10;
	self.TrackStart = math.random(5,15)/10;
	self.AntiRandomness = 1-self.Randomness;
	self.Radius = Lib.CFG:Get("drone","radius",200)/4;
	self.Damage = Lib.CFG:Get("drone","damage",150)/2;
	self.CanTrack = false;
...
function ENT:StartTouch(e,delay_deletion)
	...
	if(e and e:IsValid()) then
		...
		if(class == "drones" or class == "minidrone") then return end;
...
language.Add("minidrone",Lib.Language.GetMessage("mdrone_kill"));
end
