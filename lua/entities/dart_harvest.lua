...
function ENT:Initialize()
	...
	-- Use the config @aVoN
	self.MaxEnts = Lib.CFG:Get("harvester","max_ents",5);
	self.AllowConstrained = Lib.CFG:Get("harvester","allow_constrained",false);
	self.AllowPlayers = Lib.CFG:Get("harvester","allow_players",true);
	self.AllowFrozen = Lib.CFG:Get("harvester","allow_frozen",false);
	self.ConsumeEnergy = Lib.CFG:Get("harvester","energy",100);
	self.Disallowed = {};
	for _,v in pairs(Lib.CFG:Get("harvester","disallowed_entities",""):TrimExplode(",")) do
...
function ENT:Allowed(e)
	...
	if(not (self.Ents[e] or self.Disallowed[e:GetClass()] or e.IsStargate or e.IsDHD or e.IsRings or e:IsPlayer() and e:HasGodMode() or e==game.GetWorld() or e:GetModel():find("*"))) then -- "*" in modelname is always a brush/map entity
		local allow = hook.Call("Lib.Harvester.Ent",nil,e,self);
...
ENT.Beam = Lib.MaterialCopy("HarvesterBeam","models/alyx/emptool_glow");
ENT.LightMaterial = Lib.MaterialFromVMT(
	"HarvesterSprite",
	[["UnLitGeneric"
	{
...
function ENT:Think()
	if(self.Entity:GetNetworkedBool("on",false)) then
		if(not Lib.VisualsMisc("cl_harvester_dynlights")) then return end;
		if((self.NextLight or 0) < CurTime()) then -- Fixes a crashing bug, which spawns more and more lights all over the time until the clientside "overflowed blubb" message appears
			self.NextLight = CurTime()+0.001;
		 	...
		 	if(dlight) then
				...
				local trace = Lib.Trace:New(self.Entity:GetPos(),self:GetBeamNormal(),self.Entity)
