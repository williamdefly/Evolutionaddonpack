...
function ENT:Setup(start, dir, speed, time, effect)
	...
	util.Effect("energy_beams",fx, true, true);
		...
		if self.StargateTrace.Hit then
			...
			if dist < self.Speed then
				...
				if IsValid(ent) then
					...
					if (class == "shields" or class == "shieldcore_bubble" or class == "shield_ship" and (not IsValid(ent.Parent) or not ent.Parent.Depleted)) then
...
function ENT:DoUsualHit()
	...
	if (self.Effect == "AG3") then
		if not self.BlastCreated then
			local ent = ents.Create("sat_blastwave");
	...
	elseif (self.Effect == "Ori") then
		...
		util.Effect("energy_impacts",fx,true,true);
	else
		local fx = EffectData();
			fx:SetOrigin(self.StargateTrace.HitPos);
			fx:SetNormal(self.StargateTrace.HitNormal);
...
language.Add("energy_beams2",Lib.Language.GetMessage("energy_beam_kill"));
end
