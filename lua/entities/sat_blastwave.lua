...
function ENT:Think()
	...
	for k,v in pairs(ents.FindInSphere(self.Entity:GetPos(), self.Radius )) do
		if IsValid(v) then
			...
			local allow = hook.Call("Lib.SatBlast.DamageEnt",nil,v);
...
function ENT:DamageShields()
	for k,v in pairs(Lib.FindEntInsideSphere(self.SplodePos, self.Radius, "shields"))	do
...
language.Add("sat_blast_wave",Lib.Language.GetMessage("sat_blastwave"));
end
