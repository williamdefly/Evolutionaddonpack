...
local function ReceiveDetonationCommand(Player, command, args)
   ...
   --DebugMsg("naquadahbomb: Received Detonation Code: "..args[2].."\n")
...
local function ReceiveAbortCommand(Player, command, args)
   ...
   --DebugMsg("naquadahbomb: Received Abort Code: "..args[2].."\n")
...
function ENT:Detonate()
   local warhead = ents.Create("gatenuke")
...
function ENT:AcceptInput(inputType, activator, caller)
	if(inputType == "Use" &&
      ...
      umsg.Start("naquadahbomb", caller)
...
language.Add("naquadahbomb", Lib.Language.GetMessage("entity_naq_bomb"))
end
...
usermessage.Hook("naquadahbomb", OnUserMessage)
...
function ENT:CreateCodeWindow()
   ...
   if (self:GetNWInt("State",0)==3) then
   	CodeWindow:SetTitle(Lib.Language.GetMessage("naq_bomb_menu_01a"))

   	CodeWindow.CodeBoxLabel = Label(Lib.Language.GetMessage("naq_bomb_menu_02a").." ", CodeWindow)
   else
   	CodeWindow:SetTitle(Lib.Language.GetMessage("naq_bomb_menu_01"))

   	CodeWindow.CodeBoxLabel = Label(Lib.Language.GetMessage("naq_bomb_menu_02").." ", CodeWindow)
   end
