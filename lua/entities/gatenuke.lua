...
DrFattyJr(Me)...
	...
	local nuke = ents.Create("gatenuke")
...
function ENT:Initialize()
	...
	util.Effect( "gatenuke_Explosion", fx)
		...
		if v:IsValid() then
			...
			elseif v:IsPlayer() then
				if v:Alive() and not v:HasGodMode() then
					local allow = hook.Call("Lib.GateNuke.KillPlayer",nil,v);
	...
	local rings = ents.Create("gatenuke_rings")
...
function ENT:Think()
	...
	ent,entpos = Lib.LOS(self.Entity, ent, entpos)
	...
	for k,v in pairs(ent) do
		...
		local allow = hook.Call("Lib.GateNuke.DamageEnt",nil,v);
		...
		if self.Rel < 5 then
			if Damage >= 250  then
				...
				elseif class == "sg_supergate" then -- i hope it will shut down gate
				...
				elseif table.HasValue(self.DHDList, class) then
					if self.Scale > 30 then v:Remove();
					elseif(not v.EAPGateSpawnerSpawned and not util.tobool(GetConVar("stargate_eap_dhd_protect"):GetInt()) or v.EAPGateSpawnerSpawned and not util.tobool(GetConVar("stargate_eap_dhd_protect_spawner"):GetInt()))then
						v:DestroyEffect()
					end
...
function ENT:DamageShields()
	for k,v in pairs(Lib.FindEntInsideSphere(self.SplodePos, self.SplodeDist, "shields"))	do
		local dmgmul = 1/(self.SplodePos:Distance(v:GetPos())*50/self.Scale)^2
		v:Hit(self.Entity, self.SplodePos, 1e7*(20-self.Rel)*dmgmul)
	end

	for k,v in pairs(Lib.FindEntInsideSphere(self.SplodePos, self.SplodeDist, "shieldcore_bubble"))	do
		local dmgmul = 1/(self.SplodePos:Distance(v:GetPos())*50/self.Scale)^2
		v:Hit(self.Entity, self.SplodePos, 1e7*(20-self.Rel)*dmgmul)
	end
end
...
language.Add("gatenuke", Lib.Language.GetMessage("gatenuke"))
end
...
function ENT:NukeSunBeams() -- All Credits for this goes to Jinto :}) -- Moustache
	...
	if not Lib.VisualsWeapons("cl_gatenuke_sunbeams") then return end
