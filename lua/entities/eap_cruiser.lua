...
function ENT:SpawnFunction(ply, tr) --######## Pretty useless unless we can spawn it @RononDex
	...
	local e = ents.Create("eap_cruiser")
...
if (StarGate and StarGate.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "eap_cruiser", StarGate.CAP_GmodDuplicator, "Data" )
end
...
ENT.PrintName = SGLanguage.GetMessage("Croiseur Wraith");
end
...
--########## Keybinder stuff
local KBD = StarGate.KeyBoard:New("WraithCruiser")
--Navigation
...
function ENT:Think()
	...
	if((GateGlider)and((GateGlider)==self)and(GateGlider:IsValid())) then
		if(p:KeyDown("WraithCruiser","Z+")) then
			self.Dist = self.Dist-5
		elseif(p:KeyDown("WraithCruiser","Z-")) then
			self.Dist = self.Dist+5
		end

		if(p:KeyDown("WraithCruiser","VIEW")) then
			if(self.FirstPerson) then
				self.FirstPerson=false
			else
		...
		if(p:KeyDown("WraithCruiser","A+")) then
			self.UDist=self.UDist+5
		elseif(p:KeyDown("WraithCruiser","A-")) then
			self.UDist=self.UDist-5
		end
	end
end
