...
function ENT:SpawnFunction( ply, tr )
	...
	if (IsValid(ply)) then
		local PropLimit = GetConVar("EAP_overloader_max"):GetInt()
		if(ply:GetCount("EAP_overloader")+1 > PropLimit) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_overloader\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			return
		end
	end
	...
	local ent = ents.Create("gates_overloader");
	ent:SetAngles(ang);
	...
	if (IsValid(ply)) then
		ply:AddCount("EAP_overloader", ent)
	end
	return ent
end
...
function ENT:PostEntityPaste(player, Ent,CreatedEntities)
	...
	local PropLimit = GetConVar("EAP_overloader_max"):GetInt()
	if(IsValid(player) and player:IsPlayer() and player:GetCount("EAP_overloader")+1 > PropLimit) then
		player:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_overloader\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		self.Entity:Remove();
		return
	end
	if (IsValid(player)) then
		player:AddCount("EAP_overloader", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "gates_overloader", Lib.EAP_GmodDuplicator, "Data" )
end
...
function ENT:IsAimedAtGate(gate)
   ...
   local gateCentre = Lib.GetEntityCentre(gate)
   ...
   self.trace = Lib.Trace:New(emitterPos,
                                   vector,
                                   ignorableEntities)
...
function ENT:UpdateWireOutputs()
   ...
   if(self.isFiring and self.remoteGate.excessPowerLimit and self.remoteGate.excessPower) then
      ...
      if(Lib.IsIrisClosed(self.remoteGate)) then
      	timeLeft = timeLeft * 2;
	  end
...
function ENT:FireBeam()
   if(self.isArmed == false || self.isActive == false ||
      ...
      Lib.IsStargateOpen(self.localGate) == false ||
      Lib.GetRemoteStargate(self.localGate) == nil) then
      self:StopFiring()
      return false
   elseif(Lib.IsIrisClosed(self.localGate)) then
      self:StopFiring()
      return false
   elseif(Lib.IsStargateOutbound(self.localGate) == false) then
      self:StopFiring()
      return false
   end
   ...
   self.remoteGate = Lib.GetRemoteStargate(self.localGate)
   ...
   for _, dhd in pairs(self.remoteGate:FindDHD()) do
      Lib.JamDHD(dhd, self.cycleInterval * 2)
   end
...
function ENT:StartFiring()
   ...
   Lib.JamRemoteGate(self.remoteGate)

   if(Lib.GetStargateEnergyCapacity(self.remoteGate) == nil) then
      Lib.SetStargateEnergyCapacity(self.remoteGate, Lib.STARGATE_DEFAULT_ENERGY_CAPACITY)
   end

   if(self.remoteGate.excessPowerLimit == nil) then
      self.remoteGate.excessPowerLimit = Lib.GetStargateEnergyCapacity(self.remoteGate) * energyMultiplier
   end
   -- fix for rd2
   if (self.remoteGate.excessPowerLimit<Lib.STARGATE_DEFAULT_ENERGY_CAPACITY) then
	  self.remoteGate.excessPowerLimit = Lib.STARGATE_DEFAULT_ENERGY_CAPACITY;
   end
...
function ENT:CreateBeam()
   inBeamInfo = EffectData()
	 ...
	 util.Effect("In_Beam", inBeamInfo,true,true)

   local gateMarker = Lib.GetGateMarker(self.localGate)
...
function ENT:HeatGate(gate)
   ...
   if(self.HasRD) then
      ...
      self:AddResource("energy", Lib.GetStargateEnergyCapacity(gate) + addedEnergy)
   ...
   if(gate.excessPower and gate.excessPowerLimit and gate.excessPower >= gate.excessPowerLimit/* && not self:FindAsuran(gate)*/) then
      ...
      util.Effect("Un_stable", overloadEffect)

      timer.Simple(30, function() if IsValid(gate) then Lib.DestroyStargate(gate) end end)
   end
...
function ENT:FindAsuran(gat)
	...
	for _,e in pairs(ents.FindByClass("asuran_gateweapon")) do
		if(IsValid(e)) then
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
   ...
   ENT.PrintName = Lib.Language.GetMessage("entity_overloader");
end
