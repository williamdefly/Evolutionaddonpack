...
function ENT:Trigger(key, value)
  if key == Lib.KeysConst[KEY_ENTER] and value and self.MenuChoosed > 0 and self.Screens[self.MenuChoosed] then
...
function ENT:PreEntityCopy()
    ...
    duplicator.StoreEntityModifier(self, "SGCMonitorInfo", dupeInfo)
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		if(ply:GetCount("EAP_sgc_screens")>=GetConVar("sbox_maxsgc_monitor"):GetInt()) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"stool_sgcscreen_limit\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			Ent:Remove();
			return
		end
		ply:AddCount("EAP_sgsgc_screens", Ent);
	end
