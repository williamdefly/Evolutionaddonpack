...
if SERVER then
  ...
  function SCR:Trigger(curr,key,value)
    ...
    if self.SelfDestructState == 1 then
      ...
      if key == Lib.KeysConst[KEY_ENTER] and value and #self.SelfDestructCode >= 1 then
        self.SelfDestructState = 2
    ...
    elseif self.SelfDestructState == 2 then
      if key == Lib.KeysConst[KEY_ENTER] and value then
        self.SelfDestructCode = ""
        self.SelfDestructState = 0
      end
      return true
    ...
    if self.Entity.Server.SelfDestruct then
      if key == Lib.KeysConst[KEY_BACKSPACE] and value then
      ...
      if key == Lib.KeysConst[KEY_ENTER] and value and #self.SelfDestructResetCode >= 1 then
        local server = self.Entity.Server
        local good = false
        --Check for good code
