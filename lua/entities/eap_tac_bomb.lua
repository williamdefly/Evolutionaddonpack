...
if SERVER then
	...
	function ENT:Initialize()
		...
		self.MaxShoots = Lib.CFG:Get("tac","max_shoots",50);
		self.MaxTargets = Lib.CFG:Get("tac","max_targets",5);
		self.ThinkInterval = Lib.CFG:Get("tac","shoot_interval",0.5);
		self.Range = Lib.CFG:Get("tac","range",800);
		self.BlastRange = Lib.CFG:Get("tac","blast_range",200);
		self.BlastDamage = Lib.CFG:Get("tac","blast_damage",200);
	...
	function ENT:Use(ply)
		if(IsValid(self.Owner) and ply == self.Owner) then
			if (!ply:HasWeapon("eap_weapon_sg_tac")) then
				ply:Give("eap_weapon_sg_tac");
				ply:GetWeapon("eap_weapon_sg_tac").Ammo = 1;
			else
				ply:GetWeapon("eap_weapon_sg_tac").Ammo = ply:GetWeapon("eap_weapon_sg_tac").Ammo+1;
			end			
			self:ResetSwep();
			self:Remove();
		end
						...
						if(IsValid(v)) then
							if(not (v==self.Owner)) then
								if(self.Kill) then
									...
									local allow = hook.Call("Lib.Tac.DamagePlayer",nil,v,self);
									...
									timer.Simple(3, function()
										if(IsValid(ent) and (ent:IsPlayer() and ent:Alive() or ent:IsNPC()) and not ent.Stunned) then
											if(IsValid(self)) then
												local allow = hook.Call("Lib.Tac.StunPlayer",nil,ent,self);
												if (allow==nil or allow) then
													self:StunEnt(ent);
													self.Stun = false;
		...
		if IsValid( Ent ) then
			if Ent:IsPlayer() and not Ent:HasGodMode() then
				if not Ent.Stunned then
					...
					Ragdoll:Spawn()
					Ragdoll:Activate()
					--Kill the player when their ragdoll is destroyed, in case a Lib eats it or something...
	...
	function ENT:Destroy()
		...
		for k,v in pairs(ents.FindInSphere(e:GetPos(),200)) do
			if(IsValid(v)) then
				if(not (v==self)) then
					local allow = hook.Call("Lib.Tac.KillOrDamage",nil,v,self);
					if (allow==nil or allow) then
						if(v:IsPlayer()) then
	...
	function ENT:ResetSwep()
		local tac = self.Owner:GetWeapon("eap_weapon_sg_tac");
		if(IsValid(tac)) then
			tac.ThrownTac = false;
			tac.CanThrow = true;
