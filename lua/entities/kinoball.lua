...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create("kinoball");
...
Lib.Teleport:Add("kinoball",ENT.FixAngles);
...
function ENT:ExitKino(ply)
	if (IsValid(ply)) then
		if (ply:Alive()) then
			...
			ply:Spawn();
			ply:SetFOV(ply.EAP_KINO_FOV or 0,0.3);
			ply:SetPos(ply.EAP_KINO_StartPos or ply:GetPos() + Vector(0,0,5)); -- whoa it repaired everythin
		end
...
hook.Add("PlayerDeath","EAP.Kino.DeathFix",function(ply)
	if (IsValid(ply)) then
		ply:SetViewEntity(ply);
		ply:SetNWBool("KActive", false);
		ply:SetNWEntity("Kino", NULL)
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		local PropLimit = Lib.CFG:Get("kino_dispenser","max_kino",4);
		if(ply:GetCount("EAP_kino")+1 > PropLimit) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_kino\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			self.Entity:Remove();
			return
		end
	end
	...
	if (IsValid(ply)) then
		self.Owner = ply;
		ply:AddCount("EAP_kino", self.Entity)
	end
end

if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "kinoball", Lib.EAP_GmodDuplicator, "Data" )
end
...
language.Add("kinoball", Lib.Language.GetMessage("entity_kino"))
end
