...
ENT.Model = Model("models/ship/traveler.mdl")
...
function ENT:Think()
	...
	self.ExitPos = self:GetPos()-self:GetRight()*500+self:GetForward()*200;
	...
	if(IsValid(self.Pilot)) then
		...
		if(self.Pilot:KeyDown("EAP_KEYBOARD","FIRE")) then
			if(self.CanShootStuff) then
				...
				timer.Create("travelerCanShoot"..self:EntIndex(),self.TimeBetweenEachStuffShoot,1,function()
					self.CanShootStuff = true
				end)
			end
		end
...
function ENT:OnRemove()
	if timer.Exists("travelerCanShoot"..self:EntIndex()) then timer.Destroy("travelerCanShoot"..self:EntIndex()); end
...
function ENT:FireBlast(diff)
	...
	local e = ents.Create("energy_pulse_traveler");
...
--function DrawHUD() -- Draw that HUD @Elanis and damlanil

	--local ply = LocalPlayer();
	--local self = ply:GetNetworkedEntity("ScriptedVehicle", NULL)
    --local vehicle = ply:GetNWEntity("traveler")
	...
	--if (self and self:IsValid() and vehicle and vehicle:IsValid() and self==vehicle) then
		...
		--	local health = math.Round(self:GetNetworkedInt("health")/18000*100) -- test fix
			--draw.WordBox(8,hudpos.healthw,hudpos.healthh, "Health: "..health.."%","ScoreboardText",Color(50,50,75,0), Color(255,255,0,255))

	--end

--end
--hook.Add("HUDPaint","DrawHUDtraveler",DrawHUD);
