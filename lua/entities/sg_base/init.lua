...
if(CurTime() > 10) then Lib.CallReload(_) end;
...
function ENT:Initialize()
	--################# Config
	self.DHDRange = Lib.CFG:Get("dhd","range",1000); -- Maximum range a DHD can be in before the gate says "Hey, you do not belong to me, get the fuck out"
	self.DialBlocked = Lib.CFG:Get("stargate","dial_blocked",false);
	self.DialBlockedWorld = Lib.CFG:Get("stargate","world_blocked",false);
	self.WormHoleJumpNDMG = Lib.CFG:Get("stargate","wormholejump",3000);
	self.EnergyTransInterval = Lib.CFG:Get("stargate","en_trans_interval",0.1);
	self.MaxEnergyTransfer = Lib.CFG:Get("stargate","max_energy_transfer",80000);
	self.EnergyTransfer = Lib.CFG:Get("stargate","energy_transfer",true);
	self.ResTransInterval = Lib.CFG:Get("stargate","res_trans_interval",0.1);
	self.MaxResTransfer = Lib.CFG:Get("stargate","max_res_transfer",5000);
	self.ResTransfer = Lib.CFG:Get("stargate","resource_transfer",true);

	self.ResTransferClasses = {};
	local classes = Lib.CFG:Get("stargate","res_classes","");
	if (classes!="") then
	...
	if (self.Entity:GetClass()!="sg_supergate") then
		self:Spark();
		Lib.UpdateGateTemperatures(self);
	end
	...
	if (groupsystem) then
		...
		elseif (self:GetGateAddress()!="") then
			for _,v in pairs(ents.FindByClass("sg_*")) do
				if (self.Entity != v.Entity and v.IsStargate and v:GetClass()!="sg_supergate" and v:GetGateAddress()!="") then
...
function ENT:OnRemove()
	Lib.StopUpdateGateTemperatures(self);
...
function ENT:ConvarsThink(send)
	...
	local convar = GetConVar("stargate_eap_candial_groups_dhd"):GetInt()
	...
	convar = GetConVar("stargate_eap_candial_groups_menu"):GetInt()
	...
	convar = GetConVar("stargate_eap_energy_dial"):GetInt()
	...
	convar = GetConVar("stargate_eap_energy_dial_spawner"):GetInt()
	...
	convar = GetConVar("stargate_eap_dhd_destroyed_energy"):GetInt()
	...
	convar = GetConVar("stargate_eap_atlantis_override"):GetInt()
...
function ENT:SubFlicker(target,jump,super)
	...
	elseif(self.Entity:GetClass() == "sg_infinity" and not self.InfDefaultEH)then
	...
	if (target) then
		...
		elseif(self.Target.Entity:GetClass() == "sg_infinity" and not self.InfDefaultEH) then
...
function ENT:Flicker(magnitude)
	...
	if(self.IsOpen) then
		...
		local target = false
		if (IsValid(self.Target) and self.Target:GetClass()!="sg_supergate" and IsValid(self.Target.EventHorizon) and self.Target.EventHorizon:IsOpen()) then target = true end
...
util.AddNetworkString("Lib.VGUI.Menu");

--#################  Use - Open the Dial Menu @aVoN
function ENT:Use(p)
	if self.Cooldown or self.jammed then return end;
	if(IsValid(p) and p:IsPlayer()) then
		if(hook.Call("Lib.Player.CanDialGate",GAMEMODE,p,self.Entity) == false) then return end; -- Not allowed to dial!
		if (self.DisMenu) then return end;
		local groupmenus = GetConVar("stargate_group_system"):GetBool();
		local alternatemenu = GetConVar("stargate_eap_different_dial_menu"):GetBool();
		local candialg = GetConVar("stargate_eap_candial_groups_menu"):GetBool();
...
function ENT:TriggerInputDefault(k,v,mobile,mdhd)
	...
	elseif(k == "Dial Address") then
		if ((self:GetWire("Dial Address",0) >= 1 or mobile and IsValid(mdhd) and mdhd:GetWire("Dial Address",0) >= 1) and not self.Active and self:CheckEnergy(true,true)) then
			local candialg = GetConVar("stargate_eap_candial_groups_wire"):GetInt()
			if (self:GetLocale()==true or self.Entity:GetClass()=="sg_supergate") then candialg = 0 end
			local char = string.char(v):upper();
			local a = self.DialledAddress;
			if(v==Lib.Keyboard.Constants[ENTER] and (#a >= 6 and #a <= 9 and candialg==1 or #a == 6 or #a == 7 and a[7] == "#")) then -- Enter Key
				...
				if (#self.DialledAddress >= 7 and #self.DialledAddress <= 9) then
					...
					if(self.Entity:GetClass() == "sg_orlin") then fast = true end; -- orlin is ALWAYS dialling fast!
					if(self.Entity:GetClass()=="sg_supergate" and nox) then nox = false; fast = true end
					if (nox) then
			...
			elseif ((v == Lib.Keyboard.Constants[ENTER] or v == Lib.Keyboard.Constants[BACKSPACE]) and self.Active and not self:IsShutdown()) then
				self:AbortDialling();
			end
		end
...
function ENT:IsConceptDHD()
	...
	if (self.Entity:GetClass()=="sg_atlantis" or self.Entity:GetClass()=="sg_universe") then return true end
...
function ENT:Ramps(t)
	...
	if(RampOffset[mdl]) then
		...
		for _,v in pairs(Lib.GetConstrainedEnts(e,2) or {}) do
...
function ENT:CheckRamp()
	if (self.Entity:GetClass()=="sg_orlin"||self.Entity:GetClass()=="sg_supergate") then return false; end;
...
function ENT:TransferResource(resname,value)
	...
	if self.IsOpen and IsValid(self.Target) and self.Target.IsOpen then
		...
		if (Lib.RD.Connected(target) and origin:GetResource(resname)>0) then
