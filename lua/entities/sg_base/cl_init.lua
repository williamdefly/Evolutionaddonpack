...
function ENT:SetGateAddress(s)
	if(s and (s:len() == 6 or s == "")) then
		LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Address \""..s.."\"");
	end
end
...
function ENT:SetGateGroup(s)
	if(s and (s:len() == 2 or s:len() == 3 and self.Entity:GetClass() == "sg_universe")) then
		LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Group \""..s.."\"");
	end
end
...
function ENT:SetGateName(s)
	if(s) then
		LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Name \""..s.."\"");
	end
end
...
function ENT:SetPrivate(b)
	LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Private "..tostring(b));
end
...
function ENT:SetLocale(b)
	LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Locale "..tostring(b));
end
...
function ENT:SetGalaxy(b)
	LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Galaxy "..tostring(b));
end
...
function ENT:SetBlocked(b)
	LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Blocked "..tostring(b));
end
...
function ENT:DialGate(address,mode,nox)
	...
	if (nox) then
		LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." NoxDial "..address);
	else
		LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." Dial "..tostring(mode).." "..address);
	end
end
...
function ENT:AbortDialling()
	LocalPlayer():ConCommand("_Lib.SetValue "..self.Entity:EntIndex().." AbortDialling true");
end
...
function ENT:Draw()
	...
	for i=1,9 do
		if(self.Entity:GetNWBool("chevron"..i,false)) then
			...
			if Lib.LOSVector(EyePos(), endpos, LocalPlayer(), 10) then
...
function ENT:Think()
	...
	if(Lib.VisualsMisc==nil or not Lib.VisualsMisc("cl_stargate_dynlights")) then return end;
...
hook.Add("HUDPaint","Lib.Hook.HUDPaint.ShowAddressAndGroupAndName",
	function()
				...
				if(trace.Hit and IsValid(trace.Entity)) then
					...
					if(e.IsGroupStargate and e:GetNetworkedBool("SG_GROUP_SYSTEM")) then
						...
						if(address != "" and group != "") then
							...
							local message = Lib.Language.GetMessage("stargate_address")..": "..address.." - "..Lib.Language.GetMessage("stargate_group")..": "..group.." - "..Lib.Language.GetMessage("stargate_name")..": "..name;
							if (e:GetClass()=="sg_universe") then
								message = Lib.Language.GetMessage("stargate_address")..": "..address.." - "..Lib.Language.GetMessage("stargate_type")..": "..group.." - "..Lib.Language.GetMessage("stargate_name")..": "..name;
							end
							draw.WordBox(8,40,ScrH()/2,message,"Default",Color(50,50,75,100),color);
						end
					elseif(e.IsGroupStargate and e:GetClass()!="sg_universe") then
						local address = e:GetGateAddress();
						if(address ~= "") then
							local name = e:GetGateName();
							if(name == "") then name = "N/A" end;
							local galaxy = e:GetGalaxy();
							if galaxy then galaxy = Lib.Language.GetMessage("stargate_galaxy_y")
							else galaxy = Lib.Language.GetMessage("stargate_galaxy_n") end
							local message = Lib.Language.GetMessage("stargate_address")..": "..address.." - "..Lib.Language.GetMessage("stargate_name")..": "..name.." - "..Lib.Language.GetMessage("stargate_galaxy")..": "..galaxy;
							draw.WordBox(8,40,ScrH()/2,message,"Default",Color(50,50,75,100),color);
						end
					elseif(e.IsStargate) then
						...
						if(address != "") then
							local name = e:GetGateName();
							if(name == "") then name = "N/A" end;
							local message = Lib.Language.GetMessage("stargate_address")..": "..address.." - "..Lib.Language.GetMessage("stargate_name")..": "..name;
							draw.WordBox(8,40,ScrH()/2,message,"Default",Color(50,50,75,100),color);
						end
					end
				end
...
function ENT:FindPowerDHD()
	...
	for _,v in pairs(posibble_dhd) do
		if (v:GetClass()=="dhd_atl_city" or IsValid(v:GetNWEntity("LockedGate")) and v:GetNWEntity("LockedGate")!=self.Entity) then continue end
		local e_pos = v:GetPos();
		local dist = (e_pos - pos):Length(); -- Distance from DHD to this stargate
		if(dist <= self.Entity:GetNetworkedInt("DHDRange",1000)) then
			-- Check, if this DHD really belongs to this gate
...
function ENT:CheckConnection(chevs)
	...
	if (chevs==9 or Sclass == "sg_supergate") then self.ConnectionSGU = true
