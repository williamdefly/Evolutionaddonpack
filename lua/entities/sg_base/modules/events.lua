...
function ENT.Sequence:OpenGate()
	...
	local autoclose = Lib.CFG:Get("stargate","autoclose_time",38);
...
function ENT:StartDialling()
	self:FindGate();
	local allow = hook.Call("Lib.Dial",GAMEMODE,self.Entity,self.Target or NULL,table.concat(self.DialledAddress or {}):sub(1,9),self.DialType);
	if(allow == false or GetConVar("stargate_group_system"):GetBool() and self:GetGateGroup()=="" and self.Entity:GetClass()!="sg_supergate") then return end;
