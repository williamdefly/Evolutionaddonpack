...
function ENT:WireStartDialling()
	self:FindGate();
	local allow = hook.Call("Lib.Dial",GAMEMODE,self.Entity,self.Target or NULL,table.concat(self.DialledAddress or {}):sub(1,9),self.DialType);
		...
		if(self.IsOpen) then
			...
			if(inbound or (self.DialledAddress and (#self.DialledAddress >= 8 and #self.DialledAddress <= 10) and (inbound or not self:IsBlocked(nil,nil,true)))) then
				...
				if(not DEBUG and not fail) then -- No debug, no instant open
					local dly = self:GetDelayNN(self.Target:GetClass(),self.Target.Classic);
					if (not inbound) then
			...
			if (candialg==1) then
				for k,v in pairs(gates) do
					if(IsValid(v) and v ~= self.Entity and not v:GetPrivate()) then
						...
						if(address != "" and group != "" and IsValid(ent) and (not locale and not ent:GetLocale() or (ent:GetGateGroup() == group or v:GetClass()=="sg_universe" and ent:GetClass()=="sg_universe")) and (address!=ent:GetGateAddress() or group!=ent:GetGateGroup())) then
							...
							if (ent:GetGateGroup() != group and (v:GetClass()!="sg_universe" or ent:GetClass()!="sg_universe") or c_range > 0 and range>c_range and ent:GetGateGroup():len()==3) then
				...
				for k,v in pairs(gates) do
					if(IsValid(v) and v ~= self.Entity and not v:GetPrivate()) then
						...
						if(address != "" and group != "" and IsValid(ent) and (address!=ent:GetGateAddress() or group!=ent:GetGateGroup())) then
							local range = (ent:GetPos() - v:GetPos()):Length();
							local c_range = ent:GetNetworkedInt("SGU_FIND_RANDE"); -- GetConVar("stargate_sgu_find_range"):GetInt();
							if ((ent:GetGateGroup() == group or v:GetClass()=="sg_universe" and ent:GetClass()=="sg_universe") and (range<=c_range and ent:GetGateGroup():len()==3 or ent:GetGateGroup():len()==2 or c_range == 0 and ent:GetGateGroup():len()==3)) then
								local name = v:GetGateName();
								if(name == "") then name = "N/A" end;
								local blocked = 0;
								if (v:GetBlocked()) then blocked = 1; end
				...
				if(IsValid(v) and v ~= self.Entity and not v:GetPrivate()) then
					...
					if(address ~= "") then
					    ...
					    if(v:GetGalaxy() or g:GetGalaxy() or
						   v:GetClass()=="sg_universe" and g:GetClass()=="sg_universe" and
						   c_range > 0 and range>c_range)then
						    address = address.."@";
					    end
					    if(v:GetClass() == "sg_atlantis" and g:GetClass() == "sg_atlantis" and #address == 7 and g:GetGalaxy() and v:GetGalaxy())then
							address = string.Explode("@",tostring(address));
							address = address[1];
						end
						if(#address == 7 and g:GetGalaxy() and v:GetGalaxy() and ((v:GetClass() ~= "sg_atlantis" and g:GetClass() ~= "sg_atlantis") and
						   (v:GetClass() ~= "sg_universe" and g:GetClass() ~= "sg_universe")))then
							address = string.Explode("@",tostring(address));
							address = address[1];
						end
						if((v:GetClass() == "sg_universe" and g:GetClass() ~= "sg_universe") or --#address == 7 and
						   (v:GetClass() ~= "sg_universe" and g:GetClass() == "sg_universe"))then
							address = string.Explode("@",tostring(address));
							address = address[1].."@!";
						end
...
function ENT:GetGates()
	local class = "sg_*"
	if (self.Entity:GetClass()=="sg_supergate") then class = "sg_supergate" end
	local stargate = ents.FindByClass(class);
	local gates = {};
	for _,v in pairs(stargate) do
		if (v.IsStargate and self.Entity:GetClass()=="sg_supergate" or v.IsGroupStargate) then
