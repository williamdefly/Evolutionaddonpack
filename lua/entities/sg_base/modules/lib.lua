...
function ENT:CheckConnection()
	...
	if (#self.DialledAddress==10 or Sclass == "sg_supergate") then self.ConnectionSGU = true
	else self.ConnectionSGU = false end
	if self.ConnectionSGU then issgu = 1; end
	if (Sclass == "sg_supergate") then issgu = 2; end
...
function ENT:WireGetEnergy(addr,dist)
	...
	if (self:GetClass()=="sg_supergate") then issgu = 2 end
	...
	if(#address==8 and self:GetClass()!="sg_supergate")then consume = consume / self.ChevConsumption end;
	if (issgu) then
		consume = consume*2;
	else
		consume = consume*5;
	...
	--  energy :)
	energy = consume;
	if(#address==8 and self:GetClass()!="sg_supergate") then
		for k,v in pairs(self:FindPowerDHD()) do -- look for other power sources -- look for dhd :)
...
function ENT:WormHoleJump()
	if (self.IsOpen and self.Outbound and IsValid(self.EventHorizon) and self.Entity:GetClass() != "sg_orlin" and not self.IsSupergate and (not IsValid(self.Target) or not self.Target.jammed)) then
		...
		for _,v in pairs(ents.FindByClass("sg_*")) do
			if ((v.IsGroupStargate or v:GetClass() == "sg_supergate") and v~=self.Entity and v~=old and not v.IsOpen and not v.IsDialling and v:GetClass() != "sg_orlin") then
		...
		if gate:GetClass() == "sg_supergate" then
			gate:InstantLightUp();
		else
...
function ENT:FindPowerDHD()
	if (IsValid(self.LockedDHD) and (not self.LockedDHD.Destroyed or util.tobool(GetConVar("stargate_eap_dhd_destroyed_energy"):GetInt()))) then 
		if self.LockedDHD.Disabled then return {} end
		return {self.LockedDHD} 
	end
	...
	if(self.Entity:GetClass() == "sg_universe")then
	    for _,v in pairs(Lib.GetConstrainedEnts(self.Entity,2) or {}) do
            if(IsValid(v) and not v:GetClass():find("sg_"))then
   	            if(Lib.RampOffset.Gates[v:GetModel()])then
				    table.insert(dhd,v);
					return dhd;
     			end
			end
	...
	for _,v in pairs(posibble_dhd) do
		if (v:GetClass()=="dhd_atl_city" or IsValid(v.LockedGate) and v.LockedGate!=self.Entity) then continue end
		local e_pos = v:GetPos();
		local dist = (e_pos - pos):Length(); -- Distance from DHD to this stargate
		...
		if(dist <= range) then
			...
			if (v:GetClass():find("dhd_") and v.Destroyed and not util.tobool(GetConVar("stargate_eap_dhd_destroyed_energy"):GetInt())) then
				add = false;
			end
			if(add) then
				table.insert(dhd,v);
	...
	if(not (self.IsOpen or self.Dialling) or no_target) then
		...
		if(dialadr and (#dialadr >= 8 and #dialadr <= 10)) then
			...
			for _,v in pairs(self:GetAllGates()) do
				if(v ~= self.Entity) then
					...
					if (#dialadr == 8 and
						caddress and
						a[7] == "#" and
						(group == g or self.Entity:GetClass()=="sg_universe") and
						(self.Entity:GetClass()!="sg_universe" or
						c_range == 0 or
						c_range > 0 and
						group:len()==3 and
						g:len()==3 and
			...
			if(#gates >= 1) then
				...
				if (#gates > 1 and GetConVar("stargate_eap_atlantis_override"):GetInt()>=1) then
					for k,v in pairs(gates) do
						if (IsValid(v) and v:GetClass()=="sg_atlantis") then
							target = gates[k];
							break;
						end
					end
			...
			for _,v in pairs(self:GetAllGates()) do
				if(v ~= self.Entity) then
					...
					if (#dialadr == 8 and
						...
						c_range == 0 or
						c_range > 0 and
						v:GetClass()=="sg_universe" and
						self.Entity:GetClass()=="sg_universe" and
						range<=c_range)
					) then
						table.insert(gates,v);
					elseif (#dialadr == 9 and
						...
						a[7] == "@" and
						a[8] == "#" and
						(g!=galaxy or v:GetClass()=="sg_universe" and
						self.Entity:GetClass()=="sg_universe" and
						c_range > 0 and
						range>c_range)
					) then
					...
					elseif (#dialadr == 10 and
						...
						self.Entity:GetClass()!="sg_universe" or
						v:GetClass()!="sg_universe" and
						self.Entity:GetClass()=="sg_universe")
					) then
						table.insert(gates,v);
					end
				elseif(#dialadr == 10 and string.Implode("",a)==self.ScrAddress.."DIAL") then
...
function ENT:SetGateAddress(s)
	if (self.Entity:GetClass()=="sg_orlin") then return end
	...
	if(s:len() == 6 or s == "") then
		...
		address = hook.Call("Lib.SetGateAddress",GAMEMODE,self.Entity,self.GateAddress or "",address) or address;
...
function ENT:SetGateGroup(s)
	if (self.Entity:GetClass()=="sg_supergate") then return end
	if (self.Entity:GetClass()=="sg_universe") then
	...
	if(s:len() == 2 and self.Entity:GetClass() != "sg_universe" or s:len() == 3 and self.Entity:GetClass() == "sg_universe") then
		local group = s:upper();
		if not self:CheckGroup(s,self.GateAddress) then return end
		group = hook.Call("Lib.SetGateGroup",GAMEMODE,self.Entity,self.GateGroup or "",group) or group;
...
function ENT:SetGateName(s)
	if(s) then
		s = hook.Call("Lib.SetGateName",GAMEMODE,self.Entity,self.GateName or "",s) or s;
...
function ENT:SetPrivate(b)
	b = util.tobool(b);
	local override = hook.Call("Lib.SetPrivate",GAMEMODE,self.Entity,self.GatePrivat or false,b);
...
function ENT:SetLocale(b)
	if (self.Entity:GetClass()=="sg_supergate" or self.Entity:GetClass()=="sg_orlin") then return end
	...
	b = util.tobool(b);
	local override = hook.Call("Lib.SetLocal",GAMEMODE,self.Entity,self.GateLocal or false,b);
	if(type(override) == "boolean") then b = override end;
	self.GateLocal = b;
...
function ENT:SetGalaxy(b)
	...
	b = util.tobool(b);
	local override = hook.Call("Lib.SetGalaxy",GAMEMODE,self.Entity,self.GateGalaxy or false,b);
	if(type(override) == "boolean") then b = override end;
	self.GateGalaxy = b;
...
function ENT:GetBlocked()
	if (self:GetNetworkedInt("SG_BLOCK_ADDRESS")<=0 or self:GetNetworkedInt("SG_BLOCK_ADDRESS")==1 and self:GetClass()!="sg_universe") then return false; end
...
function ENT:SetBlocked(b)
	if (self.Entity:GetClass()=="sg_supergate" or self.Entity:GetClass()=="sg_orlin") then return end
	b = util.tobool(b);
	local override = hook.Call("Lib.SetBlocked",GAMEMODE,self.Entity,self.GateBlocked or false,b);
	if(type(override) == "boolean") then b = override end;
	self.GateBlocked = b;
...
concommand.Remove("_Lib.SetValue"); -- In case of a sent_reload (or lua_reload? How is that new command named?)
concommand.Add("_Lib.SetValue",
	function(p,_,arg)
		...
		if(IsValid(e) and c and d) then
			...
			elseif(c == "Dial" and d2) then
				...
				if(e:GetClass() == "sg_orlin") then b = true end; -- SGA is ALWAYS dialling fast! - FIXME: Add new dialling to this gate (with the new sounds!)
				/*if (e.NoxDialingType == true) then //nox dialing type
					e:NoxDialGate(d2);
				else */
					e:DialGate(d2,b);
				--end
				hook.Call("Lib.Player.DialledGate",GAMEMODE,p,e,d2,b);
			elseif(c == "NoxDial") then
				e:NoxDialGate(d);
				hook.Call("Lib.Player.DialledGate",GAMEMODE,p,e,d,true,true);
			elseif(c == "AbortDialling") then
				e:AbortDialling();
				hook.Call("Lib.Player.ClosedGate",GAMEMODE,p,e);
			end
		end
	end
);
...
function ENT:IsNewDial(class)
	if (class == "sg_sg1"||class == "sg_infinity"||class == "sg_movie"||class == "sg_universe") then
		return true;
	end
	return false
end
