...
local MCDEntities = {
	...
	["naquadah_generator_mk1"] = {
		["check"] = "naquadah_generator_mks",
		["stop"] = 33,
	},
	["replicator"] = {
...
function ENT:Initialize()
	...
	self.Speed = Lib.CFG:Get("mcd","speed",100)/100;
	self.CheckRights = Lib.CFG:Get("mcd","check_rights",true);
...
function ENT:SpawnFunction(p,t)
	...
	if (IsValid(p)) then
		local PropLimit = GetConVar("EAP_mcd_max"):GetInt()
		if(p:GetCount("EAP_mcd")+1 > PropLimit) then
			p:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_mcd\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			return
		end
	end
...
function ENT:Use(ply)
    if(not self.Create and self.Forw)then
		...
		for k,v in pairs(MCDEntities) do
			...
			if (not Lib.NotSpawnable(v.check or k,pl,v.type or "tool",true)) then
				classes[k] = true;
			end
		end
...
function ENT:Think()
	...
	elseif(not self.Create and self.Undone)then
		...
		if (IsValid(self.Ent)) then
			...
			if (class=="naquadah_generator_mk1") then class = "naquadah_generator_mks"; end
			undo.Create(class)
...
function ENT:StartCreate(class,owner,data)
	...
	local pl = owner;
	if (not self.CheckRights) then pl = nil end
	if (class!="replicator" and Lib.NotSpawnable(info.check or class,pl,info.type or "tool")) then return end	
	local realclass = nil
	if (class=="replicator") then realclass = "prop_ragdoll"; class = "prop_physics"; end
	if (class=="eap_fnp90" or class=="eap_weapon_zat" or class=="eap_weapon_asura" or class=="eap_sg_medkit" or class=="eap_weapon_gdo") then realclass = class; class = "prop_physics"; end
    local e = ents.Create(class);
	if (not IsValid(e)) then return end
	self:SetWire("Creation Class",orig_class);
    if (class=="zpmmk3" and Lib.CFG:Get("mcd","allow_tzmp",false)) then
    	local rnd = Lib.CFG:Get("mcd","tzmp_chance",2);
    ...
    if(class == "tollan_weapons_disabler" or class == "cloakinggenerator" or class == "shields_generator" or class == "jammingdevice")then
        ...
        if(class == "tollan_weapons_disabler" or class == "jammingdevice")then
	    ...
	    elseif(class == "cloakinggenerator" or class == "shields_generator")then
...
function ENT:PostEntityPaste(player, Ent,CreatedEntities)
	...
	local PropLimit = GetConVar("EAP_mcd_max"):GetInt()
	if(IsValid(player) and player:IsPlayer() and player:GetCount("EAP_mcd")+1 > PropLimit) then
		player:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_mcd\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		self.Entity:Remove();
		return
	end
	if (IsValid(player)) then
		player:AddCount("EAP_mcd", self.Entity)
		self.Owner = player;
	end
	Lib.RD.PostEntityPaste(self,player,Ent,CreatedEntities)
	Lib.Wire.PostEntityPaste(self,player,Ent,CreatedEntities)
end

if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "mcd", Lib.EAP_GmodDuplicator, "Data" )
end