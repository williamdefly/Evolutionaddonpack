...
function ENT:Initialize()
	...
	self.MaxEnergy = Lib.CFG:Get("naquadah_bottle","capacity",30000);    --@meeces2911 WHAT is this energy ment to be, it was set to 500!? that it not very much energy ... ? Did you want 500 NE ?
	self.MaxStorage = Lib.CFG:Get("naquadah_bottle","energy_capacity",10000);
	self.enabled = false;
...
function ENT:SpawnFunction(p,t)
	...
	local e = ents.Create("naq_bottle");
...
function ENT:Think()
	...
	if(self.depleted or not self.HasResourceDistribution) then self.Entity:NextThink(CurTime()+0.5); return true; end;
		...
		if(not self.disabled) then
			self.resfix = false;
			if(Lib.RD.Connected(self)) then -- We are connected to a network - Enable Naquadah
...
language.Add("naq_bottle",Lib.Language.GetMessage("stool_naq_bottle"));
end
