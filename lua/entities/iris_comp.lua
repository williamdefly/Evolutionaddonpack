...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_iris_computer_max"):GetInt()
	if(ply:GetCount("EAP_iris_computer")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_iris_pc\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y+180) % 360;

	local ent = ents.Create("iris_comp");
	ent:SetAngles(ang);
	ent:SetPos(tr.HitPos+Vector(0,0,20));
	ent:Spawn();
	ent:Activate();
	ent.Owner = ply;
	ply:AddCount("EAP_iris_computer", ent)
...
local function gdopc_menuhook(len)
	...
	codeLabel:SetText(Lib.Language.GetMessage("iriscomp_code"))

	local descLabel = vgui.Create("DLabel" , DermaPanel )
	descLabel:SetPos(120,25)
	descLabel:SetText(Lib.Language.GetMessage("iriscomp_desc"))
	...
	codeList:AddColumn(Lib.Language.GetMessage("iriscomp_code"))
	codeList:AddColumn(Lib.Language.GetMessage("iriscomp_desc"))
	...
	closetime:SetText( Lib.Language.GetMessage("iriscomp_time") )
    closetime:SetMin( 0 )
    closetime:SetMax( 10 )
	closetime:SetValue( closetimeval );
    closetime:SetDecimals( 0 )
	closetime:SetToolTip(Lib.Language.GetMessage("iriscomp_time_desc"))
	...
	autoclose:SetText(Lib.Language.GetMessage("iriscomp_close"))
	autoclose:SizeToContents()
	autoclose:SetPos(20, 240)
	autoclose:SetValue( autocloseval )
	autoclose:SizeToContents()
	autoclose:SetTooltip(Lib.Language.GetMessage("iriscomp_close_desc"))

	local autoopen = vgui.Create("DCheckBoxLabel" , DermaPanel )
	autoopen:SetText(Lib.Language.GetMessage("iriscomp_open"))
	autoopen:SizeToContents()
	autoopen:SetPos(150, 240)
	autoopen:SetValue( donotautoopen )
	autoopen:SetTooltip(Lib.Language.GetMessage("iriscomp_open_desc"))
	...
	ToggleIris:SetText(Lib.Language.GetMessage("iriscomp_toggle"))
