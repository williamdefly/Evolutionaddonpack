...
ENT.PrintName = "Vaisseau Mère D anubis"
ENT.Author = ""
ENT.Spawnable = false //Disabled since a model added
...
ENT.Model = Model("models/skybox/mship2.mdl")

ENT.Sounds = {
	Staff=Sound("eap/ship/armes/AnubisStaff.wav"),
}
...
function ENT:SpawnFunction(ply, tr) --######## Pretty useless unless we can spawn it @RononDex
	...
	local e = ents.Create("eap_mothership")
...
function ENT:Initialize() --######## What happens when it first spawns(Set Model, Physics etc.) @RononDex
	...
	self:SetNetworkedInt("CanFire",1)
...
function ENT:FireBlast(diff)
	if(self.Delay>=10) then
		...
		local e = ents.Create("energy_pulse_mothership");
...
if (StarGate and StarGate.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "eap_mothership", StarGate.CAP_GmodDuplicator, "Data" )
end
...
ENT.PrintName = SGLanguage.GetMessage("mothership");
end
...
--########## Keybinder stuff
local KBD = StarGate.KeyBoard:New("AnubisMotherShip")
--Navigation
...
ENT.Sounds={
	Engine=Sound("eap/ship/moteur/anubis.mp3"),
}
...
function ENT:Think()
	...
	if((GateGlider)and((GateGlider)==self)and(GateGlider:IsValid())) then
		if(p:KeyDown("AnubisMotherShip","Z+")) then
			self.Dist = self.Dist-5
		elseif(p:KeyDown("AnubisMotherShip","Z-")) then
			self.Dist = self.Dist+5
		end

		if(p:KeyDown("AnubisMotherShip","VIEW")) then
			if(self.FirstPerson) then
				self.FirstPerson=false
			else
		...
		if(p:KeyDown("AnubisMotherShip","A+")) then
			self.UDist=self.UDist+5
		elseif(p:KeyDown("AnubisMotherShip","A-")) then
			self.UDist=self.UDist-5
		end
	end
end
