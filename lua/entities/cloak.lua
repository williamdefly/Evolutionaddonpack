...
function ENT:Initialize()
	...
	-- local classnames = Lib.CFG:Get("cloaking","classnames",""):lower();
	self.Disallowed = {};
	-- for _,v in pairs(classnames:TrimExplode(",")) do
	-- 	self.Disallowed[v] = true;
	-- end
	-- local exceptions = Lib.CFG:Get("cloaking","exceptions",""):lower();
	self.Exceptions = {};
	...
	if CPPI then
		...
		if(owner and owner:IsValid() and owner:IsPlayer() and owner.CPPIGetFriends) then
			...
			table.insert(tbl,owner); -- Temporary add to debug visibility
