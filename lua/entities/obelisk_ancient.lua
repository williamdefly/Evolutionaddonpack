...
if (Lib.Langauge!=nil and Lib.Langauge.GetMessage!=nil) then
	ENT.Category = Lib.Langauge.GetMessage("entity_main_cat");
	ENT.PrintName = Lib.Langauge.GetMessage("entity_obelisk");
end
...
function ENT:Initialize()
	...
	for _,v in pairs(ents.FindByClass("obelisk_ancient")) do
		if (v != self) then
			CurrentToSend = false;
		end
	end
	...
	if (not timer.Exists("Obelisk_TeleportFunc")) then
		timer.Create("Obelisk_TeleportFunc", 60, 0, function()
			local obelisks = ents.FindByClass("obelisk_ancient")
...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_obelisk_ancient_max"):GetInt()
	if(ply:GetCount("EAP_obelisk_ancient")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Langauge.GetMessage(\"entity_limit_anc_obelisk\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y+180) % 360;

	local ent = ents.Create("obelisk_ancient");
	ent:SetAngles(ang);
	...
	ply:AddCount("EAP_obelisk_ancient", ent)
	return ent
end
...
function ENT:FindGate()
	...
	for _,v in pairs(ents.FindByClass("sg_*")) do
		if(v.IsGroupStargate and v:GetClass()!="sg_orlin" and v:GetGateAddress()!="") then
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("Count_sod_obelisk", self.Entity)
		self.Owner = ply;
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "obelisk_ancient", Lib.EAP_GmodDuplicator, "Data" )
end
