...
if (Lib!=nil and Lib.Wire!=nil) then Lib.Wiremod(ENT); end -- When you need to add LifeSupport and Wire capabilities, you NEED TO CALL this before anything else or it wont work!
ENT.Type = "anim"
	...
	if(b) then
		if(not self:Enabled() and not self.CantBeEnabled) then
			...
			if((not self.Depleted or (self.Strength >= self.RestoreThresold)) and self.Strength > 0) then
				...
				local e = ents.Create("shield_ship");
				--e.Size = self.Size;
