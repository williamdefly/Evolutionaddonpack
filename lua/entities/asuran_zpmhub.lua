...
function ENT:Initialize()
	self.Entity:SetModel("models/pg_props/pg_stargate/pg_asuran_hub.mdl");
...
function ENT:SpawnFunction(p,t)
	...
	local e = ents.Create("asuran_zpmhub");
	e:SetPos(pos);
	e:SetAngles(ang);
	e:DrawShadow(true);
	e:SetVar("Owner",p);
...
function ENT:Touch(ent)
	...
	if (self.CanEject == true and ent.IsZPM and ent ~= self.ZPM.Ent) then
		if (not self.ZPM.IsValid and self.ZPM.Eject == 0) then
			...
			ent:SetPos(pos + self.Entity:GetRight()*(self.Position.R) + self.Entity:GetUp()*(39.1+10) + self.Entity:GetForward()*(self.Position.F));
			ent:SetAngles(ang);
...
function ENT:ZPMsMovement()
	...
	if (self.ZPM.IsValid and self.ZPM.Dist ~= self.ZPM.Dir) then
		...
		self.ZPM.Ent:SetPos(pos + self.Entity:GetRight()*(self.Position.R) + self.Entity:GetUp()*(36.1+13*self.ZPM.Dist) + self.Entity:GetForward()*(self.Position.F));
...
function ENT:EjectZPM()
	if (self.ZPM.Ent) then
		...
		local pos = self.Entity:GetPos();
		self.ZPM.Ent:SetPos(pos + self.Entity:GetRight()*(self.Position.R*mul) + self.Entity:GetUp()*(39.1+12) + self.Entity:GetForward()*(self.Position.F*mul));
	end
	self.ZPM.Ent = nil;
...
language.Add("asuran_zpmhub",Lib.Language.GetMessage("stool_asuran_hub"));
end

if (Lib==nil or Lib.MaterialFromVMT==nil) then return end

ENT.ZpmSprite = Lib.MaterialFromVMT(
	"ZpmSprite",
	[["Sprite"
	{
...
ENT.Zpm_hud = surface.GetTextureID("VGUI/resources_hud/asuran_hub");
...
function ENT:Draw()
	...
	if(not Lib.VisualsMisc("cl_draw_huds",true)) then hook.Remove("HUDPaint",tostring(self.Entity).."SGAsuranH"); return end;
	local ang=EyeAngles()
    ang.y = ang.y;
	...
	hook.Remove("HUDPaint",tostring(self.Entity).."SGAsuranH");
    if(LocalPlayer():GetEyeTrace().Entity == self.Entity && EyePos():Distance( self.Entity:GetPos() ) < 1024)then
		hook.Add("HUDPaint",tostring(self.Entity).."SGAsuranH",function()
		    local w = 0;
            local h = 260;
    	    ...
    	    draw.DrawText("Asuran HUB", "header", ScrW() / 2 + 54 + w, ScrH() / 2 +41 - h, Color(0,255,255,255),0);
    	    if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
            	draw.DrawText(Lib.Language.GetMessage("hud_status"), "center2", ScrW() / 2 + 40 + w, ScrH() / 2 +65 - h, Color(209,238,238,255),0);
		    	draw.DrawText(Lib.Language.GetMessage("hud_energy"), "center2", ScrW() / 2 + 40 + w, ScrH() / 2 +115 - h, Color(209,238,238,255),0);
