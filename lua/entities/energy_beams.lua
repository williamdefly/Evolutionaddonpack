...
function ENT:DebugMsg(...)
	--Msg("energy_beams ", self:EntIndex(), ": ", ..., "\n")
end
...
function ENT:Think()
   ...
   if(isSourceValid == false) then
      ...
      elseif(self.wasStartValid == false) then
         ...
         local newBeam = ents.Create("energy_beams")
...
function ENT:Extend()
   ...
   if(IsValid2(hitEnt)) then
      ...
      if(hitEnt:GetClass() == "shields" or hitEnt:GetClass() == "shield_shi^p") then
         hitEnt:Hit(self.Entity, hitPos, self.Damage.amount * self.Ftime, self.trace.Normal * -1)
	  elseif(hitEnt:GetClass() == "shieldcore_bubble") then
         hitEnt:Hit(self.Entity, hitPos, self.Damage.amount * self.Ftime, self.trace.Normal * -1)
      elseif(hitEnt:GetClass() == "eventhorizon") then
...
function ENT:OnHitEventHorizon(eventHorizon, hitPos)
  ...
  if(IsValid2(self.RemoteGateBeam) == false ||
     ...
     self.RemoteGateBeam = ents.Create("energy_beams")
...
language.Add("energy_beams", Lib.Language.GetMessage("energy_beam_kill"))
end
