...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_ioncannon_max"):GetInt()
	if(ply:GetCount("EAP_ioncannon")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_toll_ion\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = ang.y % 360

	local ent = ents.Create("turret_tollan");
	ent:SetPos(tr.HitPos);
	ent:SetAngles(ang);
	ent:Spawn();
	ent:Activate();

	ply:AddCount("EAP_ioncannon", ent)
	ent:SpawnRest(ply);
	ent.Duped = true;
	return ent
end
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("EAP_ioncannon", self.Entity)
	end
	self.Duped = true;
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "turret_tollan", Lib.EAP_GmodDuplicator, "Data" )
end
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	ENT.Category = Lib.Language.GetMessage("cat_weapons");
	ENT.PrintName = Lib.Language.GetMessage("entity_ion_cannon");
end
