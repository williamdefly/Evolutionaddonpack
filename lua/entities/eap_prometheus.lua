...
function ENT:SpawnFunction(ply, tr) --######## Pretty useless unless we can spawn it @RononDex
	...
	local e = ents.Create("eap_prometheus")
...
function ENT:Think()
	...
	if((GateGlider)and((GateGlider)==self)and(GateGlider:IsValid())) then
		if(p:KeyDown("prometheus","Z+")) then
			self.Dist = self.Dist-5
		elseif(p:KeyDown("prometheus","Z-")) then
			self.Dist = self.Dist+5
		end

		if(p:KeyDown("prometheus","VIEW")) then
			if(self.FirstPerson) then
				self.FirstPerson=false
			else
		...
		if(p:KeyDown("prometheus","A+")) then
			self.UDist=self.UDist+5
		elseif(p:KeyDown("prometheus","A-")) then
			self.UDist=self.UDist-5
		end
	end
end
