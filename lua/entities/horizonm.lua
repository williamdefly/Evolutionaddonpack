...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_horizon_max"):GetInt()
	if(ply:GetCount("EAP_horizon")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_horizon\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ent = ents.Create("horizonm");
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (Lib.NotSpawnable(self:GetClass(),ply)) then self.Entity:Remove(); return end
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("EAP_horizon", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "horizonm", Lib.EAP_GmodDuplicator, "Data" )
end
...
function ENT:FireMissile()
	...
	for i=1,10 do
		...
		ent = ents.Create("horizonmissile");
		ent.Parent = ent;
		ent.Track = true;
		ent.Target = pos;
		ent.IsDecoy = decoy;
...
ENT.PrintName = Lib.Language.GetMessage("entity_horizon_missile");
end
