...
ENT.Author = "Madman07, Iziraider, Rafael De Jongh"
ENT.Instructions= ""
...
function ENT:Initialize()
	...
	self.HealthWep = Lib.CFG:Get("ori_satellite","health",500);
	self.WepPower = 100;
	self.Shield = nil;
	self.Strength = 100;
...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_ori_max"):GetInt()
	if(ply:GetCount("EAP_ori")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_ori_sat\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	ent = ents.Create("satellite_ori");
	...
	ply:AddCount("EAP_ori", ent)
	return ent
end

function ENT:SpawnShield()
	local e = ents.Create("shields");
	e.Size = 240;
	e.DrawBubble = true;
...
function ENT:LowProrityThink()
	...
	if (CurTime() - self.Time > 1) then
		local ShieldTime = Lib.CFG:Get("ori_satellite","shield_time",120);
		local WeaponTime = Lib.CFG:Get("ori_satellite","recharge_time",60);
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		self.Owner = ply;
		ply:AddCount("EAP_ori", self.Entity)
	end

end

if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "satellite_ori", Lib.EAP_GmodDuplicator, "Data" )
end
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	...
	ENT.PrintName = Lib.Language.GetMessage("entity_ori_satellite");
end

ENT.GlowSprite = Material("effects/multi_purpose_noz"); //MaterialFromVMT doesnt support changing render mode of sprities! @Mad
...
function ENT:Draw()
	...
	if Lib.LOSVector(EyePos(), endpos, {LocalPlayer(), self.Entity}, 50) then
