...
local protected_entities = {
	...
	atlantis_trans=true,
	atlantis_trans_doors=true,
	-- rg_* and sg_* protected too
}

function ENT:SpawnFunction(p,tr)
	if (not tr.Hit) then return end
	local e = ents.Create("atlantis_trans");
...
function ENT:Initialize()
	...
	self.Disallowed = {};
	for _,v in pairs(Lib.CFG:Get("atlantis_trans","classnames",""):TrimExplode(",")) do
...
function ENT:CreateDoors(ply,spawner,protect)
	...
	local d = ents.Create("atlantis_trans_doors");
	local e = self.Entity
	...
	d.BaseTP = self;
	local d2 = ents.Create("atlantis_trans_doors");
	...
	local d = ents.Create("doors_contr");
	local e = self.Entity
	...
	if (name!="") then
		...
		for _,v in pairs(ents.FindByClass("atlantis_trans")) do
			if(v.TName == name and v.Entity != self.Entity) then
				if (not wire) then ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"atl_tp_error\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )"); end
				return;
			end
		end
...
function ENT:FindTransporter(name)
	if name=="" or not name then
		...
		local nEnt
		local entt=ents.FindByClass("atlantis_trans")
	...
	if (IsValid(self.Target)) then
		...
		for _,v in pairs(ents.FindInBox(min,max)) do
			if IsValid(v) and not self.Target.EntsTP[v] then
				...
				if v:IsPlayer() then
					local allow = hook.Call("Lib.AtlantisTransporter.TeleportEnt",nil,v,self);
