...
function ENT:CAPOnShieldTouch(shield)
	self:Blast("eap_energy_impact",self:GetPos(),shield,Vector(1,1,1),false,self.Damage,self.Radius);
	if (self.Explosion) then self:Blast("eap_energy_explosion",self:GetPos(),self,Vector(1,1,1),false,self.Damage,self.Radius); end
	self:Destroy();
end
...
function ENT:Explode()
	self:Blast("eap_energy_explosion",self:GetPos(),self,Vector(1,1,1),false,self.Damage,self.Radius);
	self:Destroy();
end
...
function ENT:PhysicsCollide( data, physobj )
	...
	if (e) then
		...
		if self.Stunner then
			...
			self:Blast("eap_energy_impact",pos,e,hitnormal,hitsmoke,0,0, data.OurOldVelocity);
		else
			self:Blast("eap_energy_impact",pos,e,hitnormal,hitsmoke,self.Damage,self.Radius, data.OurOldVelocity);
		end
		if (self.Explosion) then self:Blast("eap_energy_explosion",pos,self,Vector(1,1,1),false,self.Damage,self.Radius); end
...
Lib.Teleport:Add("eap_energy_pulse",ENT.FixAngles);
...
language.Add("eap_energy_pulse",Lib.Language.GetMessage("eap_energy_pulse_kill"));
end
...
function ENT:Initialize()
	self.Created = CurTime();
	self.DrawShaft = true;
	self.InstantEffect = not (self.Entity:GetClass() == "eap_energy_pulse");
