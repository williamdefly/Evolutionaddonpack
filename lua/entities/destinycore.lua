...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_applecore_max"):GetInt()
	if(ply:GetCount("EAP_applecore")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_app_core\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y-60) % 360;

	local ent = ents.Create("destinycore");
	ent:SetAngles(ang);
	...
	ply:AddCount("EAP_applecore", ent)
	return ent
end
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (Lib.NotSpawnable(Ent:GetClass(),ply)) then self.Entity:OnRemove(); return end
	local PropLimit = GetConVar("EAP_applecore_max"):GetInt();
	if (IsValid(ply)) then
		if(ply:GetCount("EAP_applecore")+1 > PropLimit) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_app_core\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
	...
	if (IsValid(ply)) then
		self.Owner = ply;
		ply:AddCount("EAP_applecore", self.Entity)
	end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "destinycore", Lib.EAP_GmodDuplicator, "Data" )
end
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	...
	ENT.PrintName = Lib.Language.GetMessage("entity_apple_core");
end
...
function ENT:Think()
	...
	if (Lib.VisualsMisc("cl_applecore_smoke") and self.Wire > 0 and CurTime() > self.SmokeTime) then
		self.SmokeTime = CurTime()+0.2;
		self:Smoke();
	end
	if (Lib.VisualsMisc("cl_applecore_light") and self.Wire > 0) then
