...
language.Add("dakarawave", Lib.Language.GetMessage("dakara_energy_kill"))
end
...
function ENT:Initialize()
    ...
    util.Effect("dakarawave", effectInfo);
end
   ...
   for _, entity in pairs(ents.FindInSphere(self:GetPos(), self.radius)) do
    local isValidTarget = (self:IsValidTarget(entity) && entity:GetPhysicsObject():IsValid()) -- So it doesn't dissolve stuff like info player start.
		...
		if (isValidTarget) then
			local allow = hook.Call("Lib.DarakaWave.Disintegrate",nil,entity,self);
      ...
      if isValidTarget then
         ...
         if(entity:GetClass() ~= "shields" || self.IsShieldPiercing == true) then
...
function ENT:PropagateThroughGate(gate)
	for _,v in pairs(ents.FindByClass("sg_*")) do
		if(v.IsStargate and v!=gate and v:GetClass() != "sg_supergate" and v:GetClass() != "sg_orlin" and v:GetClass() != "sg_universe" and not v.GateGalaxy) then
			local wave = ents.Create("dakarawave")
...
function ENT:HitShields()
   for _, entity in pairs(ents.FindInSphere(self:GetPos(), self.radius)) do
      if(entity.isHitByDakaraWave ~= true) then
         if(self.IsShieldPiercing == false && entity:GetClass() == "shields") then
