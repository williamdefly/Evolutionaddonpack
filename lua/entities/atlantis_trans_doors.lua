...
usermessage.Hook("Lib.AtlantisTP.ClipStart", function(um)
...
usermessage.Hook( "Lib.AtlantisTP.ClipStop", function(um)
	local e = um:ReadEntity();
	if (not IsValid(e)) then return end
	e.ClipEnabled = nil;
		...
		if self.Open then
			timer.Create("Close2"..self:EntIndex(),self.Delay,1,function()
				if (IsValid(self)) then
					self.Open = false;
					umsg.Start("Lib.AtlantisTP.ClipStop");
						umsg.Entity(self);
					umsg.End()
				end
			end);
			...
			umsg.Start("Lib.AtlantisTP.ClipStart");
				umsg.Entity(self);
			umsg.End()
