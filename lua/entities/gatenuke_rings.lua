...
function ENT:Initialize()
	...
	self.StartPos 	= (self.StartPos or self:GetPos() or Vector(0,0,0))
...
function ENT:Think()
	...
	for k,v in pairs (Lib.FindInsideRotatedBox(self.StartPos, -1*offset, offset, self.ANG)) do
		if v:IsValid() then
			if v:GetPhysicsObject():IsValid() then
				table.insert(Ents, v)
				table.insert(EntsPos, v:GetPos())
	...
	local IsInShield = Lib.ArePointsInsideAShield(EntsPos)
				...
				if (dis < self.Rad+150) and (dis > self.Rad-150) then
					if e:IsPlayer() then
						if e:Alive() and not e:HasGodMode() then
							local allow = hook.Call("Lib.GateNuke.KillPlayer",nil,e);
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	language.Add("gatenuke_rings", Lib.Language.GetMessage("gatenuke"))
end
