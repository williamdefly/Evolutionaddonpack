...
ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "eap_cloaking"
ENT.Author = "aVoN"
ENT.WireDebugName = "eap_cloaking"
...
if (Lib==nil or Lib.MaterialCopy==nil) then return end
ENT.Material = Lib.MaterialCopy("CloakMuzzle","effects/strider_bulge_dudv");
...
hook.Add("PrePlayerDraw","Lib.Cloak", function(p)
...
-- Stops making players "recognizeable" if they are cloaked (E.g. by looking at them - Before you e.g. saw "Catdaemon - Health 100" if you lookaed at a cloaked player. Now, you dont see anything if he is cloaked
hook.Add("HUDDrawTargetID","Lib.Cloak", function()
...
if (Lib==nil or Lib.CheckModule==nil or not Lib.CheckModule("base")) then return end
...
util.AddNetworkString("Lib.Cloak.Friends");
...
function ENT:Initialize()
	...
	local classnames = Lib.CFG:Get("eap_cloaking","classnames",""):lower();
	self.Disallowed = {};
	for _,v in pairs(classnames:TrimExplode(",")) do
		self.Disallowed[v] = true;
	end
	local exceptions = Lib.CFG:Get("eap_cloaking","exceptions",""):lower();
	self.Exceptions = {};
			...
			if(IsValid(v) and not v:IsWeapon() and not self.Disallowed[class]) then
				...
				if(class ~= "prop_physics" and (not v:CreatedByMap() or movetype == MOVETYPE_VPHYSICS) or movetype == MOVETYPE_VPHYSICS and v:GetSolid() == SOLID_VPHYSICS) then
					if(not (movetype == MOVETYPE_NONE)) then -- Fix for wraith harvestere'd things
						...
						-- Add deriving entities (e.g. Lib Chevrons)
	...
	if(self.Parent.CloakAttached and constraint.HasConstraints(self.Parent)) then
		for _,v in pairs(Lib.GetConstrainedEnts(self.Parent,60)) do
			self.nocollide[v] = true;
			-- Add deriving entities (e.g. Lib Chevrons)
			for _,vv in pairs(v:GetDerived()) do
				self.nocollide[vv] = true;
	...
	-- After we got all entities for eap_cloaking, cloak them now!
...
function ENT:StartTouch(e)
	...
	if(IsValid(e.IsCloaked)) then return end; -- Cloaked by another (valid) eap_cloaking gen
	...
	-- Show eap_cloaking fade on all props
	if(self.Parent.Owner ~= e) then
		self:FieldPenetrated(e);
	end
	if(not e.IgnoreTouch) then
		self:Cloak(e,true);
		-- Cloak deriving entities of this too! (E.g. the parented chevrons on Libs)
...
function ENT:EndTouch(e)
	...
	--if(not (e.IsCloaked and e.IsCloaked:IsValid() and e.IsCloaked == self.Entity)) then return end; -- Cloaked by another (valid) eap_cloaking gen
...
function ENT:OnRemove()
	...
	local uncloak_table = table.Copy(self.nocollide); -- Must be a copy, or it will fail if we are using self.nocollide directly: We are "indexing" it and at the same time we delete stuff from it. That was the actual "not uneap_cloaking everything correctly" bug
	for k,_ in pairs(uncloak_table) do
		if(IsValid(k)) then
			self:Cloak(k,false);
			-- Uncloak deriving entities (e.g. for the chevrons on the Lib) - REMOVEd due to problems. Should not be necessary here anymore
			--[[
...
--################# Draw the eap_cloaking effect @aVoN
function ENT:eap_cloakingEffect(e,scale)
	local fx = EffectData();
	local pos = e:GetPos();
	fx:SetOrigin(pos);
	fx:SetStart(pos);
	fx:SetEntity(e);
	fx:SetScale(scale);
	util.Effect("eap_eap_cloaking",fx,true,true);
end
...
hook.Add("PlayerDeath","Lib.Cloak.PlayerDeath",function(ply)
	ply.__SGCloacked = false;
	ply:SetNetworkedBool("CloakCloaked",false);
end)

hook.Add("PlayerSilentDeath","Lib.Cloak.PlayerDeath",function(ply)
	ply.__SGCloacked = false;
	ply:SetNetworkedBool("CloakCloaked",false);
end)

hook.Add("PlayerSwitchWeapon", "Lib.WeaponCloak.Changed", function(ply, oldWeapon, newWeapon)
