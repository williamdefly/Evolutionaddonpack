...
{
	MenuLabel	=	Lib.Language.GetMessage("stool_sgcscreen_menu"),
	Order		=	-10,
...
/*
hook.Remove("Lib.Teleport", "SGC_Computer_v1",function(ent,gate,test,blk)
...
function ENT:GetFineAddress(gate,ogate)
	...
	if grouped then
		...
		if (gate:GetClass() == "sg_universe" and ogate:GetClass() ~= "sg_universe") or	(gate:GetClass() ~= "sg_universe" and ogate:GetClass() == "sg_universe") then
			 address = address.."@!";
		elseif gate:GetClass() == "sg_atlantis" and ogate:GetClass() == "sg_atlantis" and #address == 7 and ogate:GetGalaxy() and gate:GetGalaxy() then
		elseif #address == 7 and ogate:GetGalaxy() and gate:GetGalaxy() and ((gate:GetClass() ~= "sg_atlantis" and ogate:GetClass() ~= "sg_atlantis") and (gate:GetClass() ~= "sg_universe" and ogate:GetClass() ~= "sg_universe")) then
...
function ENT:AutoConnect()
	...
	if(self.FindIDC and not IsValid(self.IDCReceiver)) then
		local fent = self:FindNearestClass("iris_comp",self:GetPos())
	...
	if(self.FindBomb and not IsValid(self.Bomb)) then
		local fent = self:FindNearestClass("naquadahbomb",self:GetPos())
		if (IsValid(fent)) then
			self.Bomb = fent
			fent.SGCScreen = self
		end
	...
	if (IsValid(gate)) and self.On and self.State == -1 then
		if CurTime()-self.AddressCheck > 10 then
			...
			for i,gate in ipairs(self.Gates) do
				...
				elseif gate:GetClass() == "sg_tollan" then
					self:SetNW2Int("AddressType"..i,5)
				elseif gate:GetClass() == "sg_atlantis" then
					self:SetNW2Int("AddressType"..i,4)
				elseif gate:GetClass() == "sg_infinity" then
					self:SetNW2Int("AddressType"..i,3)
				elseif gate:GetClass() == "sg_movie" then
		...
		local movie = self.LockedGate:GetClass() == "sg_movie"
...
function ENT:Touch(ent)
	...
	elseif not IsValid(self.IDCReceiver) and (ent.GDOStatus) and not IsValid(ent.SGCServer) then
		...
		ed:SetEntity(self)
		util.Effect("propspawn", ed, true, true)
	elseif not IsValid(self.Bomb) and ent:GetClass() == "naquadahbomb" and not IsValid(ent.SGCServer) then
		self.Bomb = ent
		ent.SGCServer = self
		local ed = EffectData()
		ed:SetEntity(self)
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	if (Lib.NotSpawnable("sgc_screen",ply,"tool")) then Ent:Remove(); return end

	if (IsValid(ply)) then
		if(ply:GetCount("CAP_sgc_servers")>=GetConVar("sbox_maxsgc_server"):GetInt()) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"stool_sgcscreen_limit_sv\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
			Ent:Remove();
			return
		end
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "sgc_server", Lib.EAP_GmodDuplicator, "Data" )
end
...
if (Lib ~=nil and Lib.Language ~=nil and Lib.Language.GetMessage ~=nil) then
  ENT.Category = Lib.Language.GetMessage("entity_main_cat")
  ENT.PrintName = Lib.Language.GetMessage("sgc_computer")
end
...
local function SGC_Codes_Mgr(otbl,ent)
	...
	DermaPanel:Center()
	--DermaPanel:SetTitle( Lib.Language.GetMessage("iriscomp_title") )
	DermaPanel:SetTitle("")
    ...
    image:SetImage("gui/cap_logo");
 	...
 	title:SetText(Lib.Language.GetMessage("sgcscreen_title"));
  	title:SetPos( 25, 0 );
 	title:SetSize( 400, 25 );

	local codeLabel = vgui.Create("DLabel" , DermaPanel )
	codeLabel:SetPos(10,185)
	codeLabel:SetText(Lib.Language.GetMessage("sgcscreen_code"))

	local descLabel = vgui.Create("DLabel" , DermaPanel )
	descLabel:SetPos(100,185)
	descLabel:SetText(Lib.Language.GetMessage("sgcscreen_desc"))
	...
	codeList:AddColumn(Lib.Language.GetMessage("sgcscreen_dcode"))
	codeList:AddColumn(Lib.Language.GetMessage("sgcscreen_desc"))
	...
	codeList:SetSize(250, 150)
	codeList:AddColumn(Lib.Language.GetMessage("sgcscreen_rcode"))
	codeList:AddColumn(Lib.Language.GetMessage("sgcscreen_desc"))
	codeList:SortByColumn(1, true)
    ...
    saveClose:SetText(Lib.Language.GetMessage("sgcscreen_save"))
    ...
    cancelButton:SetText(Lib.Language.GetMessage("sgcscreen_cancel"))
	...
	descLabel:SetText(Lib.Language.GetMessage("sgcscreen_info"))
