...
function ENT:Initialize()
	...
	self.Disallowed = {};
	for _,v in pairs(Lib.CFG:Get("ring","classnames",""):TrimExplode(",")) do
		...
		if args[1] then
			...
			for _,v in pairs(ents.FindByClass("rg_base_*")) do
				if(v.Address == adr and v.Entity != ply.RingNameEnt) then
					ply:SendLua("GAMEMODE:AddNotify(Lib.LAnguage.GetMessage(\"ring_error\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
					return;
				end
			end
...
if address=="" or not address then
	...
	local rings=ents.FindByClass("rg_base_*")
...
function ENT:DoTeleport()
	...
	for _,ent in pairs(entz) do
		-- sg_* and rg_* protect by AlexALX
		-- fixed by mad
		-- No, by AlexALX ;) xD
		...
		if not self.Disallowed[class] and not string.find(class,"sg_") and not string.find(class,"rg_") and not protected_entities[class] and ent~=game.GetWorld() and not self.Other.Ents[ent] and not self.Ents[ent] and not ent.NotTeleportable and not ent:GetParent():IsValid() and not ent.GateSpawnerProtected then
			if playersonly:GetBool() and not ent:IsPlayer() then else
				if ent:GetPhysicsObject():IsValid() then
					self.Ents[ent]=true
					local allow = hook.Call("Lib.Rings.TeleportEnt",nil,ent,self);
...
function ENT:ReadyChecks()
	...
	for _,ent in pairs(entz) do
		self.Other.Ents[ent]=true
			...
			if (self.Entity:GetClass() == "rg_base_ori") then
				for i=1,5 do
...
function ENT:StartLaser()
	...
	energyBeam:SetPos(Lib.GetEntityCentre(self.Entity));
	energyBeam:Spawn();
	energyBeam:Activate();
...
-- ################### This function originally has been added to the rg_panel. But my "rg caller" swep sets Player.RingDialEnt to the actual rg_base rather than the rg_panel entity so we need it here again to avoid lua errors @aVoN
...
function ENT:CheckRamp()
	for _,e in pairs(ents.FindInSphere(self.Entity:GetPos(),20)) do
		if (Lib.Ramps.Ring[e:GetModel()]) then
