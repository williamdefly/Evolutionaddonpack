...
function ENT:Initialize()
	...
	self.Disallowed = {};
	for _,v in pairs(Lib.CFG:Get("asgard_transporter","classnames",""):TrimExplode(",")) do
		self.Disallowed[v:lower()] = true;
	end
	self.BusyTime = Lib.CFG:Get("asgard_transporter","busy_time",5);
...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create("asgard_transporter");
	ent:SetAngles(ang);
...
function ENT:FindObjects(pos,quick)
	...
	for _,e in pairs(ents.FindInSphere(pos,130)) do
		if(e:IsValid()) then
			-- Only players?
			local allow = hook.Call("Lib.Transporter.TeleportEnt",nil,e,self);
			...
			elseif(self.TeleportEverything and not self.Exceptions[e]) then
				...
				if(self:ValidForTeleport(e)) then
					...
					local allow = true;
					if(not quick) then
						local entities = Lib.GetConstrainedEnts(e);
...
function ENT:Teleport(from,to,ply,gps_ent)
	...
	if((self.Next or 0) > time) then
		if (IsValid(ply)) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"asgardtp_busy\"), NOTIFY_ERROR, 3); surface.PlaySound( \"buttons/button2.wav\" )");
		end
		return
	end
	...
	if (not util.IsInWorld(to)) then
		if (IsValid(ply)) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"asgardtp_wrong\"), NOTIFY_ERROR, 3); surface.PlaySound( \"buttons/button2.wav\" )");
		end
		return
	end
	if not jaiming_online then
		-- Prepare teleport
		self.Exceptions = TableValuesToKeys(Lib.GetConstrainedEnts(self.Entity)); -- Not allowed because constrained to this teleporter
		...
		if(energy < needed_energy) then
			if (IsValid(ply)) then
				ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"asgardtp_energy\"), NOTIFY_ERROR, 3); surface.PlaySound( \"buttons/button2.wav\" )");
			end
			return
		end
		...
		if (IsValid(ply)) then
			ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"asgardtp_succ\"), NOTIFY_GENERIC, 4); surface.PlaySound( \"buttons/button9.wav\" )");
		end
		...
		for _,v in pairs(entities) do
			...
			util.Effect("teleporteffect",fx,true,true);
			fx:SetOrigin(dest);
	...
	elseif (IsValid(ply)) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"asgardtp_jamming\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
	end
end
...
function ENT:Use(ply)
	...
	net.Start("EAP.AsgardTransporter");
...
net.Receive("EAP.AsgardTransporter",function(len,ply)
...
net.Receive("EAP.AsgardTransporter",function(len)
	...
	local function updateAction()
		...
		if (target and dest) then
			...
			if (DermaPanel.withEnts:GetChecked()) then
				withents = "\n"..Lib.Language.GetMessage("asgardtp_acte");
			end
			if (ent.SaveTable.targetType and ent.SaveTable.targetType==3) then
				DermaPanel.action:SetText(Lib.Language.GetMessage("asgardtp_act2",tname,dname)..withents);
			else
				DermaPanel.action:SetText(Lib.Language.GetMessage("asgardtp_act",tname,dname)..withents);
			end
			DermaPanel.action:SizeToContents();
			DermaPanel.action:CenterHorizontal();
		else
			DermaPanel.action:SetText(Lib.Language.GetMessage("asgardtp_none"));
			DermaPanel.action:SizeToContents();
			DermaPanel.action:CenterHorizontal();
		end
	end
	...
	refreshButton:SetToolTip(Lib.Language.GetMessage("asgardtp_refresh"))
    ...
    targetType:AddChoice(Lib.Language.GetMessage("asgardtp_type1"),1);
    targetType:AddChoice(Lib.Language.GetMessage("asgardtp_type2"),2);
    targetType:AddChoice(Lib.Language.GetMessage("asgardtp_type3"),3);
	...
	targetType:SetToolTip(Lib.Language.GetMessage("asgardtp_target_desc"));
	...
	targetList:AddColumn(Lib.Language.GetMessage("asgardtp_target"))
    ...
    destType:AddChoice(Lib.Language.GetMessage("asgardtp_type1"),1);
    destType:AddChoice(Lib.Language.GetMessage("asgardtp_type2"),2);
    destType:AddChoice(Lib.Language.GetMessage("asgardtp_type3"),3);
	...
	destType:SetToolTip(Lib.Language.GetMessage("asgardtp_dest_desc"));
	...
	destList:AddColumn(Lib.Language.GetMessage("asgardtp_dest"))
	...
	for k,v in pairs(ents) do
 	...
 	action:SetText(Lib.Language.GetMessage("asgardtp_action"));
  	action:SetPos( 0, 280 );
 	action:SizeToContents();
	action:CenterHorizontal();

  	local act = vgui.Create( "DLabel", DermaPanel );
 	act:SetText(Lib.Language.GetMessage("asgardtp_none"));
  	act:SetPos( 0, 300 );
 	act:SizeToContents();
	...
	withEnts:SetText(Lib.Language.GetMessage("asgardtp_ents"))
	withEnts:SizeToContents()
	withEnts:SetPos(30, 260)
	withEnts:SetValue(ent.SaveTable.withEnts or 0)
	withEnts:SizeToContents()
	withEnts:SetTooltip(Lib.Language.GetMessage("asgardtp_ents_desc"))
	...
	sendButton:SetText(Lib.Language.GetMessage("asgardtp_send"))
	...
	sendButton.DoClick = function ( btn )
		...
		if (target and dest) then
			...
			GAMEMODE:AddNotify(Lib.Language.GetMessage("asgardtp_noact"), NOTIFY_ERROR, 5); surface.PlaySound( "buttons/button2.wav" );
		end
	end

	local retrieveButton = vgui.Create("DButton" , DermaPanel )
	retrieveButton:SetParent( DermaPanel )
	retrieveButton:SetText(Lib.Language.GetMessage("asgardtp_retrieve"))
