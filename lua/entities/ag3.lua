...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
	ENT.Category = Lib.Language.GetMessage("cat_weapons");
	ENT.PrintName = Lib.Language.GetMessage("entity_ag3");
end
...
function ENT:Initialize()
	...
	self.HealthWep = Lib.CFG:Get("ag3","health",500);
	self.Power = 100;
...
function ENT:FindSatellites(ent)
	...
	for _,v in pairs(ents.FindByClass("ag3")) do
...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_ag3_max"):GetInt()
	if(ply:GetCount("EAP_ag3")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_ag3\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		...
		ply:AddCount("EAP_ag3", self.Entity)
	end
	...
	if (self.Canfire) then
		...
		if self.IsMaster then
			...
			if (CurTime() - self.Time > 1) then
				local WeaponTime = Lib.CFG:Get("ag3","recharge_time",60);
				self.Time = CurTime();
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "ag3", Lib.EAP_GmodDuplicator, "Data" )
end
