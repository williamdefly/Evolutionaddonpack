...
--########## Keybinder stuff
local KBD = Lib.KeyBoard:New("Dedale")
--Navigation
...
function ENT:Think()
	...
	if((GateGlider)and((GateGlider)==self)and(GateGlider:IsValid())) then
		if(p:KeyDown("Dedale","Z+")) then
			self.Dist = self.Dist-5
		elseif(p:KeyDown("Dedale","Z-")) then
			self.Dist = self.Dist+5
		end

		if(p:KeyDown("Dedale","VIEW")) then
			if(self.FirstPerson) then
				self.FirstPerson=false
			else
		...
		if(p:KeyDown("Dedale","A+")) then
			self.UDist=self.UDist+5
		elseif(p:KeyDown("Dedale","A-")) then
			self.UDist=self.UDist-5
		end
	end
end
