...
function ENT:SpawnFunction(pl,tr)
	if(pl:GetCount("vehicle_malp")>0) then return end
	local e = ents.Create("vehicle_malp");
	...
	pl:Give("swep_tool_malp_remote")
	pl:SelectWeapon("swep_tool_malp_remote")
	pl:AddCount("vehicle_malp",e)
	return e;
end
	...
	*/
	if(IsValid(self.Controler)) then
		if(self.FirstPerson) then
			...
			if(self.Controler:KeyDown("EAP_KEYBOARD","Z+")) then
				self.Camera:SetAngles(self.Camera:GetAngles()+Angle(-1,0,0))
			elseif(self.Controler:KeyDown("EAP_KEYBOARD","A+")) then
				self.Camera:SetAngles(self.Camera:GetAngles()+Angle(0,1,0))
					self.CamStand:SetAngles(self.CamStand:GetAngles()+Angle(0,1,0))
			elseif(self.Controler:KeyDown("EAP_KEYBOARD","A-")) then
				self.Camera:SetAngles(self.Camera:GetAngles()+Angle(0,-1,0))
				self.CamStand:SetAngles(self.CamStand:GetAngles()+Angle(0,-1,0))
			elseif(self.Controler:KeyDown("EAP_KEYBOARD","Z-")) then
				self.Camera:SetAngles(self.Camera:GetAngles()+Angle(1,0,0))
			end

			if(self.Controler:KeyDown("EAP_KEYBOARD","CHGATK")) then
				self.Camera:SetAngles(self:GetAngles())
			end
		end
	end

	if(self.Control && self.Controler!=NULL && IsValid(self.Controler)) then
		if(self.Controler:KeyDown("EAP_KEYBOARD","VIEW")) then
		...
		if(self.Controler:KeyDown("EAP_KEYBOARD","EXIT")) then
			self:UnControl(self.Controler)
		end
	end
...
function ENT:PhysicOverdrive(wire)
	...
	if(not wire and self.Controler:KeyDown("EAP_KEYBOARD","LEFT") or self:GetWire("Left")>0) then trn = 70; dovel = true;
	elseif(not wire and self.Controler:KeyDown("EAP_KEYBOARD","RIGHT") or self:GetWire("Right")>0) then trn = -70; dovel = true; end

	if(not wire and self.Controler:KeyDown("EAP_KEYBOARD","FWD") or self:GetWire("Forward")>0) then spd = 160; dovel = true;
	elseif(not wire and self.Controler:KeyDown("EAP_KEYBOARD","BACK") or self:GetWire("Back")>0) then spd = -100; dovel = true; end
...
function ENT:FirstPersonSpectate(p)
	...
	self.Controler:SetNWEntity("vehicle_malp",self);
...
function ENT:StopSpectate(p)
	...
	self.Controler:SetNWEntity("vehicle_malp", NULL);
	self.Controler:SetViewEntity(self.Controler)
	self.Controler:SelectWeapon("weapon_swep_tool_malp_remote")
...
function ENT:FindGate(dist)  --######### @ aVoN
	...
	for k,v in pairs(ents.FindByClass("sg_*")) do -- Find the gates by their class name
...
function ENT:FindPlayerGate(dist)
	...
	for _,v in pairs(ents.FindByClass("sg_*")) do -- Find the gates by their class name
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply) and ply:GetCount("vehicle_malp")>0 or Lib.NotSpawnable(Ent:GetClass(),ply)) then self.Entity:Remove(); return end

	if (IsValid(ply)) then
		self.Owner = ply;
	    ply:AddCount("vehicle_malp",self)
	end
end

if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "vehicle_malp", Lib.EAP_GmodDuplicator, "Data" )
end
...
ENT.PrintName = Lib.Language.GetMessage("malp");
end

if (Lib==nil or Lib.MaterialCopy==nil or Lib.KeyBoard==nil) then return end
...
local mat = Lib.MaterialCopy("MalpBlur","pp/blurscreen");
local TEXTURES = {
...
local function RenderMALPHud()
	...
	local p = LocalPlayer();
	local malp = p:GetNWEntity("vehicle_malp")
	local pos = p:GetPos()
	local time = CurTime();
	local w,h = ScrW(),ScrH();
