...
function ENT:Initialize()
	...
	self.Entity:SetName("Sodan SodanObelisk");
...
function ENT:SpawnFunction( ply, tr )
	...
	local PropLimit = GetConVar("EAP_obelisk_sodan_max"):GetInt()
	if(ply:GetCount("EAP_obelisk_sodan")+1 > PropLimit) then
		ply:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"entity_limit_sod_SodanObelisk\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return
	end

	local ang = ply:GetAimVector():Angle(); ang.p = 0; ang.r = 0; ang.y = (ang.y+180) % 360;

	local ent = ents.Create("obelisk_sodan");
	ent:SetAngles(ang);
	...
	ply:AddCount("EAP_obelisk_sodan", ent)
	return ent
end
			...
			if (button) then
				if button == "PASS" then
					--if ply == self.Owner then
						...
						umsg.Start("SodanObeliskShowPassWindow",ply)
						umsg.Entity(self.Entity);
						umsg.End()
						ply.SodanObeliskNameEnt=self
					--end
				else
...
function ENT:Touch(ent)
	if (ent:GetClass() == "obelisk_sodan") then
...
function SetSodanObeliskPassword(ply,cmd,args)
	if ply.SodanObeliskNameEnt and ply.SodanObeliskNameEnt~=NULL then
		if args[1] then
			ply.SodanObeliskNameEnt.Password=args[1]
			ply.SodanObeliskNameEnt:SetNetworkedString("pass",args[1])
			if ply.SodanObeliskNameEnt.Paired and IsValid(ply.SodanObeliskNameEnt.Target) then
				ply.SodanObeliskNameEnt.Target.Password=args[1]
				ply.SodanObeliskNameEnt.Target:SetNetworkedString("pass",args[1])
			end
		end
		ply.SodanObeliskNameEnt=nil
	end
end
concommand.Add("setSodanObeliskpass",SetSodanObeliskPassword)
				...
				if (dist < self.Range) then
					...
					timer.Create("Transport"..v:EntIndex(), 0.5, 1, function()
						...
						local fx3 = EffectData();
							...
							fx3:SetEntity(v);
						util.Effect("arthurcloak",fx3,true);
					end)

					local fx = EffectData();
						fx:SetOrigin(v:GetShootPos()+v:GetAimVector()*10);
						fx:SetEntity(v);
					util.Effect("arthurcloak",fx,true);

					local fx2 = EffectData();
						fx2:SetEntity(v);
					util.Effect("arthurcloaklight",fx2,true);
...
function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
	...
	if (IsValid(ply)) then
		self.Owner = ply;
		ply:AddCount("EAP_obelisk_sodan", self.Entity)
	end
...
function PANEL:DoClick()
	...
	LocalPlayer():ConCommand("setSodanObeliskpass "..panel2.TextEntry:GetValue())
	panel2:Remove()
end
vgui.Register( "SodanObeliskPassButton", PANEL, "Button" )
...
function PANEL:Init()
	...
	self.Logo:SetSize(16,16);
 	...
 	self.L1:SetText(Lib.Language.GetMessage("sodan_Obelisk_menu"))
 	self.L1:SetFont("OldDefaultSmall")

 	self.Button = vgui.Create( "SodanObeliskPassButton", self)
...
vgui.Register( "SodanObeliskPassEntry", PANEL, "DFrame" )

function SodanObeliskShowPassWindow(um)
	local Window = vgui.Create( "SodanObeliskPassEntry" )
...
usermessage.Hook("SodanObeliskShowPassWindow", SodanObeliskShowPassWindow)
