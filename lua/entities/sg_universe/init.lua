...
function ENT:SpawnFunction(p,t)
	...
	local e = ents.Create("sg_universe");
	e:SetPos(pos);
	e:DrawShadow(false);
    e:Spawn();
	e:Activate();
...
function ENT:RingTickUniverse()
	--for _,self in pairs(ents.FindByClass("sg_universe")) do
	...
	if(k == "Rotate Ring" and not self.Active and (not self.NewActive or self.WireManualDial) and (not self.WireBlock or ignore)) then
		if (v >= 1) then
			if (not self.WireSpin and (self:CheckEnergy(true,true) or self.WireManualDial)) then
				...
				self.WireBlock = true;
				if (timer.Exists("Li.Universe.WireBlock_"..self.Entity:EntIndex())) then
					timer.Remove("Lib.Universe.WireBlock_"..self.Entity:EntIndex());
				end
				timer.Create("Lib.Universe.WireBlock_"..self.Entity:EntIndex(), 0.1, 1, function ()
					if (IsValid(self.Entity)) then
		...
		elseif (self.WireSpin) then
			...
			self:SetSpeed(false);
			self.WireBlock = true;
			if (timer.Exists("Lib.Universe.WireBlock_"..self.Entity:EntIndex())) then
				timer.Remove("Lib.Universe.WireBlock_"..self.Entity:EntIndex());
			end
			timer.Create("Lib.Universe.WireBlock_"..self.Entity:EntIndex(), 1.0, 1, function ()
				if (IsValid(self.Entity)) then
					self.WireBlock = false;
				end
			end );
...
function ENT:BearingSetSkin(BearingLight)
    if(IsValid(self.Entity))then
	    local delay = 0;
	    for k,v in pairs(Lib.GetConstrainedEnts(self.Entity,2) or {}) do
		    if(IsValid(v))then
...
function ENT:SguRampSetSkin(rampchevlight,rampchevlightoff)
    if(IsValid(self.Entity))then
	    local delay = 0;
	    for k,v in pairs(Lib.GetConstrainedEnts(self.Entity,2) or {}) do
		    if(v:IsValid() and v:GetClass():find("sguramp"))then
...
function ENT:OnRemove()
	...
	if (self.HasRD) then Lib.RD.OnRemove(self) end;
