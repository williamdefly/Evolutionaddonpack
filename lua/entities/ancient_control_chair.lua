...
ENT.PrintName = Lib.Language.GetMessage("ancient_control_chair");
end
...
function ControlCHCalcView(Player, Origin, Angles, FieldOfView)
	...
	if(not IsValid(chair) or self:GetClass()!="ancient_control_chair") then return end;
...
function ENT:DynLight()
	...
	if(IsValid(self)) then
		if(Controlling) then
			if(Lib.VisualsMisc("cl_chair_dynlights")) then
...
function ENT:SpawnFunction(pl, tr)
	...
	local e = ents.Create("ancient_control_chair")
...
if (Lib and Lib.EAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "ancient_control_chair", Lib.EAP_GmodDuplicator, "Data" )
end
