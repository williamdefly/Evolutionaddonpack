...
SWEP.PrintName = Lib.Language.GetMessage("weapon_misc_virus");
SWEP.Category = "EAP";
end
SWEP.Category = "EAP";
...
SWEP.WElements = {
	["World_weapon_model"] = { type = "Model", model = "models/assassin21/agv/agv.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.2, 9.2, -0.4), angle = Angle(-120, 180, 0), size = Vector(0.75, 0.75, 0.75), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:SpawnProp()
	...
	if(IsValid(tr.Entity) and not tr.Entity:GetClass():find("sg_") or tr.StartPos:Distance(pos)>75)then
     return;
	end

	local PropLimit = GetConVar("EAP_agv_max"):GetInt()
	if(IsValid(p) and p:IsPlayer() and p:GetCount("EAP_agv")+1 > PropLimit) then
		p:SendLua("GAMEMODE:AddNotify(\"AGV limit reached!\", NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return;
	end

	local ent = ents.Create("ashen_virus");
	ent:SetPos(pos);
	ent:SetAngles(ang);
