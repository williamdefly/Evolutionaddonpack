...
hook.Add("EntityTakeDamage", "Lib.PersonalShield.StopDamage",
	function(ent, dmginfo)
		if ent and ent:IsValid() and ent:IsPlayer() and ent.pShielded and ent:HasWeapon("eap_personal_shield") then
...
timer.Create("Lib.PersonalShield.Think",0.1,0,
	function()
		for _,v in pairs(player.GetAll()) do
			local valid = v:IsValid()
			local hw = v:HasWeapon("eap_personal_shield")
...
local function ConCommandShieldEngage(ply)
	local weapon = ply:GetWeapon("eap_personal_shield")
