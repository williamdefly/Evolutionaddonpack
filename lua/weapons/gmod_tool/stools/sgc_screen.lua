...
TOOL.Name=Lib.Language.GetMessage("stool_sgcscreen");
...
TOOL.Topic["name"] = Lib.Language.GetMessage("stool_sgcscreen_spawner");
TOOL.Topic["desc"] = Lib.Language.GetMessage("stool_sgcscreen_create");
TOOL.Topic[0] = Lib.Language.GetMessage("stool_sgcscreen_desc");
TOOL.Language["Undone"] = Lib.Language.GetMessage("stool_sgcscreen_undone");
TOOL.Language["Cleanup"] = Lib.Language.GetMessage("stool_sgcscreen_cleanup");
TOOL.Language["Cleaned"] = Lib.Language.GetMessage("stool_sgcscreen_cleaned");
TOOL.Language["SBoxLimit"] = Lib.Language.GetMessage("stool_sgcscreen_limit");
...
function TOOL:LeftClick(t)
	...
	if(p:GetCount("EAP_sgc_screens")>=GetConVar("sbox_maxsgc_monitor"):GetInt()) then
		p:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"stool_sgcscreen_limit\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return false;
	end
...
function TOOL:RightClick(t)
	...
	if(CLIENT) then return true end;
	local p = self:GetOwner();
	if(p:GetCount("EAP_sgc_servers")>=GetConVar("sbox_maxsgc_server"):GetInt()) then
		p:SendLua("GAMEMODE:AddNotify(Lib.Language.GetMessage(\"stool_sgcscreen_limit_sv\"), NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )");
		return false;
	end
	...
	if(util.tobool(self:GetClientNumber("conn_ic"))) then
		local fent = e:FindNearestClass("iris_comp",t.HitPos)
		if (IsValid(fent)) then
			e.IDCReceiver = fent
			fent.SGCScreen = e
		else
	...
	if(util.tobool(self:GetClientNumber("conn_nb"))) then
		local fent = e:FindNearestClass("naquadahbomb",t.HitPos)
		if (IsValid(fent)) then
			e.Bomb = fent
			fent.SGCScreen = e
		else
...
function TOOL:ControlsPanel(Panel)
	//Panel:NumSlider(Lib.Language.GetMessage("stool_sgcs_fps"),"sgc_screen_fps",1,100,0);
	Panel:Button(Lib.Language.GetMessage("stool_sgcscreen_menu"),"sgc_screen_menu");

	Panel:CheckBox(Lib.Language.GetMessage("stool_autoweld"),"sgc_screen_autoweld");
	Panel:CheckBox(Lib.Language.GetMessage("stool_sgcscreen_conn_sv"),"sgc_screen_conn_sv");
	Panel:AddControl("Label", {Text = Lib.Language.GetMessage("stool_sgcscreen_sv")})
	Panel:CheckBox(Lib.Language.GetMessage("stool_sgcscreen_conn_sg"),"sgc_screen_conn_sg");
	Panel:CheckBox(Lib.Language.GetMessage("stool_sgcscreen_conn_ic"),"sgc_screen_conn_ic");
	Panel:CheckBox(Lib.Language.GetMessage("stool_sgcscreen_conn_nb"),"sgc_screen_conn_nb");
	Panel:AddControl("Label", {Text = Lib.Language.GetMessage("stool_sgcscreen_cl")})
	Panel:CheckBox(Lib.Language.GetMessage("stool_sgcscreen_keyboard"),"sgc_screen_keyboard");
	Panel:CheckBox(Lib.Language.GetMessage("stool_sgcscreen_keyboard_weld"),"sgc_screen_keyboard_weld");

	local combo = {}
	combo.Label = Lib.Language.GetMessage("stool_sgcscreen_program")
	combo.MenuButton = 0
	combo.Options = {}
	...
	for k,SCR in SortedPairs(screens) do
		local name = Lib.Language.ValidMessage("stool_sgcscreen_program_"..SCR.ID) and Lib.Language.GetMessage("stool_sgcscreen_program_"..SCR.ID) or SCR.Name
	...
	Panel:AddControl("Numpad",{
		ButtonSize=22,
		Label=Lib.Language.GetMessage("stool_sgcscreen_key"),
		Command="sgc_screen_key",
	});

	Panel:AddControl("Numpad",{
		ButtonSize=22,
		Label=Lib.Language.GetMessage("stool_sgcscreen_keyd"),
	...
	Panel:AddControl("PropSelect",{Label=Lib.Language.GetMessage("stool_model"),ConVar="sgc_screen_model",Category="",Models=models});

	Panel:AddControl("Label", {Text = Lib.Language.GetMessage("stool_sgcscreen_fulldesc")})
end
