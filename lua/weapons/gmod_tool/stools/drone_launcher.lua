...
TOOL.Name=Lib.Language.GetMessage("stool_drones");
...
TOOL.Entity.Class = "drones_launcher";
...
TOOL.Topic["name"] = Lib.Language.GetMessage("stool_drones_spawner");
TOOL.Topic["desc"] = Lib.Language.GetMessage("stool_drones_create");
TOOL.Topic[0] = Lib.Language.GetMessage("stool_drones_desc");
TOOL.Language["Undone"] = Lib.Language.GetMessage("stool_drones_undone");
TOOL.Language["Cleanup"] = Lib.Language.GetMessage("stool_drones_cleanup");
TOOL.Language["Cleaned"] = Lib.Language.GetMessage("stool_drones_cleaned");
TOOL.Language["SBoxLimit"] = Lib.Language.GetMessage("stool_drones_limit");
--################# Code
...
function TOOL:ControlsPanel(Panel)
	Panel:AddControl("Numpad",{
		ButtonSize=22,
		Label=Lib.Language.GetMessage("stool_drones_shoot"),
		Command="drone_launcher_shoot",
		Label2=Lib.Language.GetMessage("stool_drones_kill"),
		Command2="drone_launcher_explode",
	});
	local auto_track = Lib.CFG:Get("drone","auto_track");
	local eye_track = Lib.CFG:Get("drone","eye_track");
	if((Lib.HasWire or auto_track) and eye_track) then
		-- Allow both tracking
		Panel:AddControl("Numpad",{
			ButtonSize=22,
			Label=Lib.Language.GetMessage("stool_drones_track"),
			Command="drone_launcher_track",
			Label2=Lib.Language.GetMessage("stool_drones_eye_track"),
			Command2="drone_launcher_eye_track",
		});
	...
	elseif(eye_track) then
		...
		Panel:AddControl("Numpad",{
			ButtonSize=22,
			Label=Lib.Language.GetMessage("stool_drones_eye_track"),
			Command="drone_launcher_eye_track",
		});
	end
	Panel:AddControl("PropSelect",{Label=Lib.Language.GetMessage("stool_model"),ConVar="drone_launcher_model",Category="",Models=self.Models});
	Panel:CheckBox(Lib.Language.GetMessage("stool_autoweld"),"drone_launcher_autoweld");
	if(Lib.HasResourceDistribution) then
		Panel:CheckBox(Lib.Language.GetMessage("stool_autolink"),"drone_launcher_autolink"):SetToolTip(Lib.Language.GetMessage("stool_autolink_desc"));
	end
end
...
if SERVER then
	...
	numpad.Register("DroneTrackOn",
		function(p,e)
			if(not (Lib.CFG:Get("drone","auto_track") or Lib.HasWire)) then return end;
			if(not e:IsValid()) then return end;
	...
	numpad.Register("DroneEyeOn",
		function(p,e)
			if(not Lib.CFG:Get("drone","eye_track")) then return end;
			if(not e:IsValid()) then return end;
			e.HasTrackedBefore = e.Track;
			e.EyeTrack = true;
			e.Track = true;
