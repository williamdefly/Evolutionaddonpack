...
function TOOL:Register()
	...
	list.Set("EAP.Tool",self.Mode,self);
	if CLIENT then
		local d = ".";
		...
		if(self.ControlsPanel) then
			self.BuildCPanel = function(Panel)
				if (Lib.CFG:Get("eap_disabled_tool",self.Mode,false)) then
					Panel:Help(Lib.Language.GetMessage("stool_disabled_tool"));
					return
				end
				-- Add the HELP, if Internet is applied!
				if(Lib.HasInternet and self.Mode) then
	...
	if SERVER then
		if(class) then
			...
			self.SpawnSENT = function(self,p,trace,...)
				...
				-- Now, spawn the SENT
				if (Lib.TOOL) then
					local e = Lib.TOOL.Entities[class].func(p,Angle(),trace.HitPos,...);
			...
			if(self.PostEntitySpawn and Lib.TOOL) then
				Lib.TOOL.Entities[class].PostEntitySpawn = function(p,e,...)
...
function TOOL:AutoLink(e1,e2)
	if(not Lib.HasResourceDistribution) then return end
