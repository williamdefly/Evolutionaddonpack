-- /*
	...
	-- Scripted by Sutich and Madman07; Sources from aVoN's Lib Mod
...
-- SWEP.PrintName = Lib.Language.GetMessage("eap_weapon_misc_kino");
...
-- function SWEP:FindGate()
	...
	-- for _,v in pairs(ents.FindByClass("sg_*")) do
		-- if (not v.IsLib or v.IsSupergate) then continue end
...
-- function SWEP:FindClosestRings()
	...
	-- for _,v in pairs(ents.FindInSphere(trace.HitPos,100)) do
		-- if(v:GetClass() == "rg_base_ancient" and not v.Busy) then
	...
	-- if(not ring) then
		-- local dist = 500;
		-- for _,v in pairs(ents.FindByClass("rg_base_ancient")) do
