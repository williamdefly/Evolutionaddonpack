...
SWEP.WElements = {
	["World_weapon_model"] = { type = "Model", model = "models/boba_fett/portable_dhd/portable_dhd.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.2, 5.8, 0.2), angle = Angle(120, 0, 0), size = Vector(0.699, 0.699, 0.699), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:SpawnProp()
	...
	if not tr.Entity:GetClass():find("sg") or not tr.Entity.IsStargate or tr.Entity.IsSupergate then return end
	...
	local ent = ents.Create("goaulddhd_prop");
	ent:SetPos(pos);
	ent:SetAngles(ang);
