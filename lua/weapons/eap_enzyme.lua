...
SWEP.WorldModel 	= "models/weapons/weapons/w_medic.mdl"
		...
		timer.Simple(1.6, function()
			if (IsValid(self) and IsValid(self.Owner)) then
				if (self.Owner:Health()<120) then
					...
					if (self.Owner:Armor()<120) then
						self.Owner:SetArmor(120)
					end
				else
					if (self.Owner:Health()<165) then
						self.Owner:SetHealth(165)
						if (self.Owner:Armor()<165) then
							self.Owner:SetArmor(165)
						end
					else
						if (self.Owner:Health()<200) then
							self.Owner:SetHealth(200)
							if (self.Owner:Armor()<200) then
								self.Owner:SetArmor(200)
							end
						else
							self.Owner:PrintMessage(HUD_PRINTTALK,"Vous avez déjà trop d'enzyme en vous, vous devez attendre")
						end
					end
...
-- Inventory Icon
if(file.Exists("materials/VGUI/weapons/enzyme_inventory.vmt","GAME")) then
	SWEP.WepSelectIcon = surface.GetTextureID("VGUI/weapons/enzyme_inventory");
end
