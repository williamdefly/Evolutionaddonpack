...
SWEP.PrintName = Lib.Language.GetMessage("eap_weapon_hand_device");
end
SWEP.Category = "EAP"
			...
			for _,v in pairs(ents.FindInSphere(pos + (100*normal),75)) do
				if(v ~= self.Owner) then
					...
					if(phys:IsValid()) then
						local allow = hook.Call("Lib.HandDevice.Push",nil,v,p);
		...
		if(ammo >= 3) then
			...
			if(IsValid(self.Target)) then
				...
				if(self.Target:IsPlayer() and not self.Target:HasGodMode()) then
					...
					--timer.Create("Lib.UnParalyze",4,1,self.Target.SprintEnable,self.Target);
					...
					timer.Create("Lib.UnParalyze",4,1,function() if IsValid(self.Entity) and IsValid(self.Target) then GAMEMODE:SetPlayerSpeed(self.Target,250,500) end end);
				end
			else
				self.Target = nil;
			end
...
function SWEP:FindClosestRings()
	...
	for _,v in pairs(ents.FindInSphere(trace.HitPos,100)) do
		if(v:GetClass():find("rg_base_*") and not v.Busy) then
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
language.Add("Battery_ammo",Lib.Language.GetMessage("naquadah"));
language.Add("weapon_hand_device",Lib.Language.GetMessage("weapon_hand_device"));
end
