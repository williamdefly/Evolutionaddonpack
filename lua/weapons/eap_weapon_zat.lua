...
SWEP.PrintName = Lib.Language.GetMessage("eap_weapon_zat");
...
function SWEP:Initialize()
	...
	self.MaxSize = Lib.CFG:Get("zat","max_size",110);
	self.KillDistance = Lib.CFG:Get("zat","kill_distance",100);
	self.DissolveDistance = Lib.CFG:Get("zat","dissolve_distance",60);
	self:SetWeaponHoldType(self.HoldType);
end
...
function SWEP:TraceForTargets(pos,normal,exception)
	local targets = {};
	local trace = Lib.Trace:New(pos,normal*8192,exception);
	if(trace.Hit and IsValid(trace.Entity)) then -- We hit one person directly.
		...
		for i=1,10 do
			...
			trace = Lib.Trace:New(origin+pos,normal*8192,exception);
...
function SWEP:DoShoot()
	...
	fx:SetRadius(64);
	util.Effect("eap_energy_muzzle",fx,true);
	--################### The Zat-Tracer
	local fx = EffectData();
		...
		if(IsValid(v) and v.ZatMode ~= 3 and not v.ZatIgnore) then -- Only valid props which are physically
			...
			if(phys:IsValid()) then
				...
				if((not v.LastZat or v.LastZat+15 < CurTime()) and not low_hp) then
					...
					if(IS_PLAYER and not v:HasGodMode()) then
						...
						--v:SprintDisable();
						--timer.Create("Lib.UnParalyze",4,1,v.SprintEnable,v);
						...
						timer.Destroy("Lib.UnParalyze"); -- Always start a fresh timer!
						timer.Create("Lib.UnParalyze",4,1,function() if IsValid(v) then GAMEMODE:SetPlayerSpeed(v,250,500) end end);
				...
				elseif((v.ZatMode == 1 or low_hp) and dist < self.KillDistance) then
					if(IS_PLAYER or IS_NPC) then
						if (IS_PLAYER and not v:HasGodMode() or IS_NPC) then
				...
				elseif(v.ZatMode == 2 and Lib.CFG:Get("zat","dissolve",true)) then
					if((not disallow_dissolve[v] and not IS_PLAYER and not self:MapObject(v) and phys:IsMoveable() and size < self.MaxSize and dist < self.DissolveDistance) or class == "prop_ragdoll") then
						...
						if(not already_checked[v] and constraint.HasConstraints(v)) then
							local entities = Lib.GetConstrainedEnts(v,8);
							if(entities) then
								local allow = true;
...
if (Lib.Language!=nil and Lib.Language.GetMessage!=nil) then
language.Add("GaussEnergy_ammo",Lib.Language.GetMessage("naquadah"));
language.Add("eap_weapon_zat",Lib.Language.GetMessage("eap_weapon_zat"));
end
