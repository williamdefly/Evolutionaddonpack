MsgN("librairies/server/general.lua")
...
function Lib.LoadConfig(p)
   if(not IsValid(p) or p:IsAdmin() or game.SinglePlayer()) then
      ...
      if (file.Exists("lua/data/eap/config.lua","GAME")) then
         file.Write("eap/config.txt",file.Read("lua/data/eap/config.lua","GAME"));
      end
      if (file.Exists("lua/data/eap/how to create your own config.lua","GAME")) then
         file.Write("eap/how to create your own config.txt",file.Read("lua/data/eap/how to create your own config.lua","GAME"));
      end
      local ini = INIParser:new("eap/config.txt",false);
      local custom_config = INIParser:new("eap/custom_config.txt",false);
