MsgN("librairies/shared/general_settings.lua")
...
net.Receive("_eap_config",function(len,ply)
	...
	if (typ==0 or typ==3 or typ==4) then
		...
		local ini = INIParser:new("eap/config.txt",false,false,false,true);
		if (not ini) then
			Lib.LoadConfig();
			ini = INIParser:new("eap/config.txt",false,false,false,true);
		end
		local custom_config = INIParser:new("eap/custom_config.txt",false,false,false,true);
	...
	elseif (typ==2) then
		...
		if (table.Count(written)>0) then
			file.Write("eap/custom_config.txt",write);
		else
			file.Delete("eap/custom_config.txt");
		end
		Lib.LoadConfig();
...
local function SG_Settings_OpenNet()
	...
	local sboxlimits = {
		...
		--{"stargate_eap_sbox_06", "bearing"},
		--{"stargate_eap_sbox_07", "brazier"},
		--{"stargate_eap_sbox_08", "cap_doors_contr"},
		--{"stargate_eap_sbox_09", "cap_doors_frame"},
		--{"stargate_eap_sbox_10", "cap_console"},
		--{"stool_controlpanel", "control_panel"},
		--{"stargate_eap_sbox_11", "drone_launcher"},
		--{"stargate_eap_sbox_12", "floorchevron"},
		{"stargate_eap_sbox_13", "sg_iris"},
		{"stargate_eap_sbox_28", "goauld_iris"},
		{"stargate_eap_sbox_14", "gravitycontroller"},
		--{"stargate_eap_sbox_15", "jamming_device"},
		--{"stargate_eap_sbox_16", "naq_gen_mks"},
		--{"stargate_eap_menu_14", "naquadah_bomb"},
		--{"stargate_eap_sbox_17", "staff_weapon_glider"},
		--{"stargate_eap_sbox_18", "cloaking_generator"},
		{"stargate_eap_sbox_19", "mobile_dhd"},
		{"stargate_eap_sbox_27", "supergate_dhd"},
		--{"stargate_eap_sbox_20", "shield_generator"},
		--{"stargate_eap_sbox_21", "tampered_zpm"},
		--{"stargate_eap_sbox_22", "tokra_emmiter"},
		--{"stargate_eap_sbox_23", "tokra_key"},
		--{"stargate_eap_sbox_24", "tollan_disabler"},
	...
	local name_tbl = {};
 	local cap_ents = {["ent"] = list.Get("EAP"),["swep"] = list.Get("EAP.Weapon")}; -- this was best my idea (create own spawnmenu tab), because now i can also use this for menu... @ AlexALX
	...
	PropertySheet:AddSheet( Lib.Language.GetMessage("stargate_menu_t1"), GroupConvarFrame, "img/eap_large_logo", false, false )
