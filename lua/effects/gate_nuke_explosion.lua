...
function EFFECT:Init(data)
	...
	if Lib.VisualsWeapons("cl_gate_nuke_plasma") then
		local fx = EffectData()
...
function EFFECT:Think()
	...
	if not Lib.VisualsWeapons("cl_gate_nuke_rings") then return true end
	...
	if (self.Relative < 10) then
		...
		for _,ang in pairs(self.Aim) do
			...
			local CannotSpawn = {}
			local draw = Lib.VisualsWeapons("cl_gate_nuke_shieldrings")
