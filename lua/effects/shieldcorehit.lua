...
EFFECT.Material1 = Lib.MaterialCopy("ShieldRefract","refract_ring");
EFFECT.Material2 = Lib.MaterialCopy("ShieldGlow","models/roller/rollermine_glow");
...
function EFFECT:Init(data)
	...
	self.DrawRadius = Lib.VisualsMisc("cl_shield_hitradius");
	self.DrawEffect = Lib.VisualsMisc("cl_shield_hiteffect");
	if(Lib.VisualsMisc("cl_shield_dynlights")) then
...
function EFFECT:Render()
	...
	if(self.Refract < 1) then
		if(self.DrawRadius) then
			...
			render.DrawQuadEasy(self.Entity:GetPos(),self.Normal,self.Size,self.Size); -- Draw from the front
			render.DrawQuadEasy(self.Entity:GetPos(),-1*self.Normal,self.Size,self.Size); -- And from the back
		end
	end
	--The glow effect
	if(self.DrawEffect) then
		self.Material2:SetVector("$color",Vector(self.Col.r/255,self.Col.g/255,self.Col.b/255));
		self.Material2:SetFloat("$alpha",math.Clamp(self.Alpha,0,1));
		render.SetMaterial(self.Material2);
		local hitsize = self.Size*2;
		render.DrawQuadEasy(self.Entity:GetPos(),self.Normal,hitsize,hitsize); -- Draw from the front
		render.DrawQuadEasy(self.Entity:GetPos(),-1*self.Normal,hitsize,hitsize); -- And from the back
	end
end
