...
function EFFECT:Init(data)
	...
	self.EH	= Lib.FindEntInsideSphere(self.Parent:GetLocalGate():GetPos(), 10, "event_horizon")[1]	-- Remote Gate
	...
	self.EndPos	= Lib.GetEntityCentre(self.EH);
...
function EFFECT:Think()
	...
	if Lib.VisualsWeapons("cl_asuran_dynlights") then
...
function EFFECT:Render()
	...
	if self.ShouldDrawBeam then
		...
		if (Lib.LOSVector(EyePos(), pos, {GetViewEntity(),self.EH}, 10)) then
	...
	if (Lib.LOSVector(EyePos(), self.MainStart, {GetViewEntity()}, 10)) then
		render.SetMaterial(self.GlowMat2);
	...
	if Lib.VisualsWeapons("cl_asuran_laser") then
		for _,startbeam in pairs(self.SmallBeamStart) do
			...
			if (Lib.LOSVector(EyePos(), startbeam, {GetViewEntity()}, 10)) then
				render.SetMaterial(self.GlowMat2);
