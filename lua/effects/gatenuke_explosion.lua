...
local matGlow = {}
	...
	matGlow.Glow2 = Lib.MaterialFromVMT(
	"GateNukeExpL",
	[["UnLitGeneric"
	{
	...
	matGlow.Glow3 = Lib.MaterialFromVMT(
	"GateNukeExpB",
	[["UnLitGeneric"
	{
...
function EFFECT:Init(data)
	...
	if (not self.Emitter) then self:Remove(); return end
	...
	if Lib.VisualsWeapons("cl_gatenuke_plasma") then
		local fx = EffectData()
		fx:SetOrigin(self.StartPos)
		fx:SetMagnitude(self.Scale)
		util.Effect("gatenuke_Explosion_Bits", fx)
	end
...
function EFFECT:Think()
	...
	if not Lib.VisualsWeapons("cl_gatenuke_rings") then return true end
	...
	if (self.Relative < 10) then
		...
		for _,ang in pairs(self.Aim) do
			...
			local CannotSpawn = {}
			local draw = Lib.VisualsWeapons("cl_gatenuke_shieldrings")

			if draw then
				CannotSpawn = Lib.ArePointsInsideAShield(spawn, 200)
			end
