...
EFFECT.GlowMat3 = Lib.MaterialFromVMT(
	"GateWeapGlowOut",
	[["UnLitGeneric"
	{
...
function EFFECT:Init(data)
	...
	self.TargetEnt	= Lib.FindEntInsideSphere(self.StartPos, 10, "eventhorizon")[1];	-- Remote Gate
	if (not IsValid(self.TargetEnt)) then return end
	self.Parent		= data:GetEntity();
	self.StartPos	= Lib.GetEntityCentre(self.Parent);
	self.Length		= 256;
...
function EFFECT:Think()
	...
	self.StartPos 	= Lib.GetEntityCentre(self.TargetEnt);
	self.AimVector	= self.TargetEnt:GetForward();
	self.StargateTrace = Lib.Trace:New(self.StartPos+self.AimVector*5, self.AimVector*1000000000, {self.Parent, self.Target});

	local iris = Lib.FindEntInsideSphere(self.StartPos, 50, "sg_iris");
...
function EFFECT:Render()
	if (self.Draw) then
		...
		if (Lib.LOSVector(EyePos(), pos, {GetViewEntity(),self.Parent}, 10)) then
