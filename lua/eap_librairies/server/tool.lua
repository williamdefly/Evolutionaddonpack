...
MsgN("eap_librairies/server/tool.lua")

Lib.TOOL = Lib.TOOL or {};
Lib.TOOL.Entities = Lib.TOOL.Entities or {};

-- Input is the SENT classname and the table keys, which shall be saved when used by duplicator (must be strings)
function Lib.TOOL.CreateSpawner(class,...)
	Lib.TOOL.Entities[class] = Lib.TOOL.Entities[class] or {};
	Lib.TOOL.Entities[class].args = {...};
	Lib.TOOL.Entities[class].func = function (p,ang,pos,...)
		...
		if(Lib.TOOL.Entities[class].PreEntitySpawn) then
			Lib.TOOL.Entities[class].PreEntitySpawn(p,e,...);
		end
		e:Spawn();
		e:Activate();
		e.Owner = p;
		-- Calls the PostEntitySpawn function. There, you can add e.g. numpad bindings and other things
		if(Lib.TOOL.Entities[class].PostEntitySpawn) then
			Lib.TOOL.Entities[class].PostEntitySpawn(p,e,...);
		end
		-- Tell the duplicator, what to save for a restore
		if(arg and type(arg) == "table") then
			for k,v in pairs(Lib.TOOL.Entities[class].args) do
				e[v] = arg[k];
			end
		end
		...
		e.UpdateKeys = function(self,...)
			...
			for k,v in pairs(argc) do
				if(v ~= _ and v ~= nil) then
					local k = Lib.TOOL.Entities[class].args[k];
					if(k) then
						self[k] = v;
					end
				end
	...
	if(duplicator and duplicator.RegisterEntityClass) then
		duplicator.RegisterEntityClass(class,Lib.TOOL.Entities[class].func,"Ang","Pos",...);
	end
end