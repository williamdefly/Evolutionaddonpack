...
MsgN("eap_librairies/shared/a_gmod13.lua")
...
-- Fix for file.* functions, needed for wiremod, finding eap e2 chips on workshop.
...
function file.Open(path,mode,param)
	...
	if (dir:upper()=="DATA" and not file.Exists(path,dir) and (mode=="r" or mode=="rb")) then
		...
		if path and (path:lower():find("^expression2/eap_shared/(.*).lua$")
		or path:lower():find("^starfall/eap_shared/(.*).lua$")) then
...
function file.Find(path,dir,order)
	...
	if (dir:upper()=="DATA") then
		...
		if path and (path:lower():find("^expression2/eap_shared/(.*)$")
		or path:lower():find("^starfall/eap_shared/(.*)$")) then
...
-- some old fonts used in eap entities
if (CLIENT) then
	local tbl = {
		font = "coolvetica",
		size = 64,
...
if (SERVER) then
	...
	function gmsave.ShouldSaveEntity(ent,t)
		if (ent.eap_NotSave) then return false end
