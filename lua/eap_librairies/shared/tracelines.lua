...
MsgN("eap_librairies/shared/tracelines.lua")
...
if SERVER then
	hook.Add("OnEntityCreated","Lib.OnEntityCreated",function(e)
		if(not IsValid(e) or not Lib.Trace) then return end;
		if(Lib.Trace.Classes[e:GetClass()]) then
			Lib.Trace:GetEntityData(e);
		end
	end)
end

--################# Add a class to check to the tracesline calculation @aVoN
function Lib.Trace:Add(c,condition)
...
function Lib.Trace:AddCondition(c,condition)
...
function Lib.Trace:InBox(pos,Min,Max)
	if(
...
Lib.InBox = function(a,b,c) return Lib.Trace:InBox(a,b,c) end;

--################# Updates the entities OBB Datas @aVoN
function Lib.Trace:GetEntityData(e)
	if(IsValid(e)) then
		local time = CurTime();
...
function Lib.Trace:HitWall(coordinate,pos,norm,mul,Min,Max,len,hit_normal)
	local old = norm;
...
function Lib.Trace:CheckCoordinate(coordinate,pos,norm,Min,Max,len,in_box)
	...
	if(in_box) then
		local mul = 0;
		if(norm[coordinate] > 0) then -- Norm == 0 has been avoided in Lib.Trace:New
	...
	for e,_ in pairs(self.Entities) do
		if(not quick_ignore[e]) then
			if(self:GetEntityData(e)) then -- Update dimension data and check, if the ent is valid!
				...
				if (class == "shieldcore_bubble") then
					in_box = Lib.IsInShieldCore(e, start);
				elseif (class == "shields") then
				...
				if (self.Classes and self.Classes[class] and (not self.Classes[class].Condition or self.Classes[class].Condition(e,{start,dir,ignore},trace,in_box))) then
					...
					if (class == "shieldcore_bubble") then // go ahead with my method @Mad
						if Lib.IsRayBoxIntersect(start, trace.HitPos, e) then // check, if we intersecting bounding box - save cpu if we are not
							local a = not in_box;
							local dir2 = dir;
							...
							hit2 = Lib.RayPhysicsPluckerIntersect(trace, dir2, e, a);
						end
					elseif (class == "tokra_shields") then // go ahead with my method @Mad
						//if Lib.IsRayBoxIntersect(start, trace.HitPos, e) then // check, if we intersecting bounding box - save cpu if we are not
							//local a = not in_box;
							////local dir2 = dir;
							//if in_box then dir2 = -1*dir end // fix shoting if we are inside, and not shape - to get hitpos on right side (not opposite)
							//if (e.ShShap == 2) then a = in_box end // small fix for box shape, i fucked triangles directions
							hit2 = Lib.RayPhysicsPluckerIntersect(trace, dir, e, true);
						...
						if (not hit and class=="shields" and not in_box and self:InBox(pos,v.Min,v.Max)) then
