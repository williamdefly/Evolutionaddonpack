...
MsgN("eap_librairies/client/keyboard.lua")
Lib = Lib or {};
Lib.KeyBoard = Lib.KeyBoard or {};


-- Saves all Schemes.
Lib.KeyBoard.Schemes = Lib.KeyBoard.Schemes or {};
Lib.KeyBoard.MouseWheel = {}; -- Used in the PlayerBindPress hook
...
for k=1,300 do
	...
	if (name and name!="") then
		Lib.KeyBoard.Keys[name:upper()] = k;
	end
end
...
for _,v in pairs(CFG) do
	if(v:find("bind%s")) then
		...
		Lib.KeyBoard.BINDS[var[2]:gsub('"',""):Trim():lower()] = var[1]:gsub('"',""):lower();
	end
end
...
function Lib.KeyBoard:New(name)
	if(Lib.KeyBoard.Schemes[name]) then return Lib.KeyBoard.Schemes[name] end; -- It already exists. Return this instead
	local t = {};
	t.Name = name;
	Lib.KeyBoard.Schemes[name] = t;
	setmetatable(t,{__index = self});
	t.__Keys = {};
	for k,v in pairs(util.KeyValuesToTable(file.Read("eap/"..name:lower()..".txt","DATA") or "")) do -- Load keysettings
...
function Lib.KeyBoard:Get(name)
	return Lib.KeyBoard.Schemes[name] or self:New(name);
end
...
--]]
function Lib.KeyBoard:CreateInstance(vehicle)
	local t = {};
	t.Parent = self;
	t.Vehicle = vehicle;
	setmetatable(t,{__index = self}); -- Index the keyboard layout, we create an instance of
...
--################### SetActive @aVoN
function Lib.KeyBoard:SetActive(b)
	if(self.Parent) then
		...
		if(self.Parent.__ActiveVehicle == self.Vehicle) then
			...
			if(not b) then self.Parent.__ActiveVehicle = nil end;
		end
	else
		self.Active = b;
	end
...
--################### GetActive @aVoN
function Lib.KeyBoard:GetActive()
	return self.Active;
end

--################### Creates one new KeyBoard Scheme.  @aVoN
-- name is that key's name. Like FWD for forward. Keep this as short as possible! key is the NAME of that key. E.g. KP_INS
function Lib.KeyBoard:SetKey(name,key)
	for k,v in pairs(self.__Keys) do
		if(v == name) then self.__Keys[k] = nil end; -- Forbid double binds
	end
	self.__Keys[key] = name;
	file.Write("eap/"..self.Name:lower()..".txt",util.TableToKeyValues(self.__Keys or {}));
end

--################### Sets a default-key of there isn#t any set yet  @aVoN
-- name is that key's name. Like FWD for forward. Keep this as short as possible! key is the NAME of that key. E.g. KP_INS
function Lib.KeyBoard:SetDefaultKey(name,key)
	local key = Lib.KeyBoard.Keys[(key or ""):upper()];
...
function Lib.KeyBoard:GetKey(key)
	for k,v in pairs(self.__Keys) do
		if(v == key) then
			return input.GetKeyName(k);
		end
	end
end
...
local function IsKeyDown(k,allow_opened_vgui)
	...
	if(not allow_opened_vgui) then
		...
		if(LastVGUIOpen) then
			if(LastVGUIOpen + 0.2 > CurTime()) then
				return false;
			end
			LastVGUIOpen = nil;
		end
	...
	elseif(k == MOUSE_WHEEL_DOWN) then
		return Lib.KeyBoard.MouseWheel["MWHEELDOWN"] or false;
	elseif(k == MOUSE_WHEEL_UP) then
		return  Lib.KeyBoard.MouseWheel["MWHEELUP"] or false;
	end
...
function Lib.KeyBoard.Think(GetKeyIsDown)
	local p = LocalPlayer();
	for name,scheme in pairs(Lib.KeyBoard.Schemes) do
		if(scheme:GetActive()) then -- Just do this if the scheme is currently active
			for k,v in pairs(scheme.__Keys) do
				local down = IsKeyDown(k);
				-- This is used in Lib.KeyBoard.PlayerBindPress to disallow a pressed bind in the case, the pressed key is used in a keyboard layout. Otherwise, you e.g. "get in noclip" and "drive forward" with your ship
				if(GetKeyIsDown) then
					if(down and input.GetKeyName(k) == GetKeyIsDown) then return true end;
				else
					if(down and not (Lib.KeyBoard.Pressed[p][name][v] == true)) then
						Lib.KeyBoard:SetKeyPressed(p,name,v);
					elseif(not down and Lib.KeyBoard.Pressed[p][name][v] == true) then
						Lib.KeyBoard:SetKeyReleased(p,name,v);
					end
				end
			end
		end
...
hook.Add("Think","Lib.KeyBoard.Think",Lib.KeyBoard.Think);

--################### For MouseWheelUp and Down @aVoN
local function UndoTimer(key,name)
	Lib.KeyBoard.MouseWheel[key] = nil;
	if(name) then
		Lib.KeyBoard:SetKeyReleased(LocalPlayer(),name,key);
	end
end

function Lib.KeyBoard.PlayerBindPress(p,bind)
	local bind = bind:Trim():lower();
	local key;
	if(bind == "invnext") then -- DOWN
		key = "MWHEELDOWN";
	elseif(bind == "invprev") then -- UP
		key = "MWHEELUP";
	end
	if(key) then
		Lib.KeyBoard.MouseWheel[key] = true;
		...
		for name,scheme in pairs(Lib.KeyBoard.Schemes) do
			if(scheme:GetActive()) then -- Just do this if the scheme is currently active
				if(scheme.__Keys[key]) then
					already_undone = true;
					Lib.KeyBoard:SetKeyPressed(p,name,key);
					timer.Simple(0.1,function() UndoTimer(key,name) end); -- Undo this. generally, one mouse-wheel action does not really take longer than 0.1 seconds
					return true;
				end
			end
		end
		if(not already_undone) then
			timer.Simple(0.1,function() UndoTimer(key) end); -- Undo this. generally, one mouse-wheel action does not really take longer than 0.1 seconds
		end
	end
	-- A key is down in this keyboard-layout. Is it because of this key here? If yes, block this bind. If not, don't do it!
	if(Lib.KeyBoard.Think(input.LookupBinding(bind))) then return true end;
end
hook.Add("PlayerBindPress","Lib.KeyBoard.PlayerBindPress",Lib.KeyBoard.PlayerBindPress);