...
MsgN("eap_librairies/vgui/stargatemenus.lua")
...
net.Receive( "RefreshGatesList", StarGateRefreshList);
...
net.Receive( "RemoveGatesFromList" , StarGateRemoveFromList );

local function StarGateRemoveList( len )
	StarGate_GetAll = {};
end
net.Receive( "RemoveGatesList" , StarGateRemoveList );
...
local VGUI
net.Receive("Lib.VGUI.Menu",function(len)
	...
	if (type<=0) then -- 0 is normal menu, -1 is alternative (without dial)
		...
		VGUI = vgui.Create("SControlerPanel");
	...
	elseif(type==1) then -- 1 is normal dial menu (used in ships/mobile dhd etc)
		...
		VGUI = vgui.Create("SControlerPanelDHD");
...
concommand.Add("stargate_reset_menu",function(ply)
	...
	RVGUI:SetCookieName("Lib.SControlerPanel_Alt");
	RVGUI:SetCookie("SG.Pos.X",nil);
	RVGUI:SetCookie("SG.Pos.Y",nil);
	RVGUI:SetCookieName("Lib.SControlerPanelDHD");
	RVGUI:SetCookie("SG.Size.W",nil);
	RVGUI:SetCookie("SG.Size.H",nil);
	RVGUI:SetCookie("SG.Pos.X",nil);
	RVGUI:SetCookie("SG.Pos.Y",nil);
...
usermessage.Hook("Lib.DialMenuDHDClose",
	function(data)
		if(VGUI and VGUI:IsValid()) then
			VGUI:SetVisible(false);
		end
...
hook.Add("GUIMousePressed","Lib.DHD.GUIMousePressed_Group",
	function(_,dir)
		--if(IsValid(DHD)) then
			local p = LocalPlayer();
			...
			if(IsValid(trace.Entity) and trace.Entity.IsDHD and not trace.Entity:GetNetworkedBool("BusyGUI",false)) then
				...
				if(btn and btn != "IRIS") then
					p:ConCommand("_Lib.DHD.AddSymbol_Group "..DHD:EntIndex().." "..btn);
					-- ######### Add/Remove symbols
					if(btn ~= "DIAL") then
...
function PANEL:SetSettings(entity,groupsystem,alternatemenu,candialg)
	...
	if (self.Alternative) then
		self:SetCookieName("Lib.SControlerPanel_Alt");
		self:SetSize(225,210);
		--self:SetMinimumSize(225,210);
	else
		self:SetCookieName("Lib.SControlerPanel");
	...
	self.VGUI = {
		...
		AddressPanel = vgui.Create("SAddressePanel",self),
		AddressLabel = {
	...
	if (entity:GetNetworkedInt("SG_BLOCK_ADDRESS")<=0 or entity:GetNetworkedInt("SG_BLOCK_ADDRESS")==1 and self.Class!="sg_universe" or self.Class=="sg_supergate") then
	...
	if (not self.Alternative) then
		self.VGUI.AddressSelect = vgui.Create("SAddresseSelect",self);
	...
	self.VGUI.TitleLabel:SetText(Lib.Language.GetMessage("stargate_vgui"));
	...
	for i=1,2 do
		local mul = (i-1);
		self.VGUI.AddressLabel[i]:SetText(Lib.Language.GetMessage("stargate_vgui_settings"));
	...
	self.VGUI.AddressPanel.OnAddressSet = function(e,address,group)
		if((self.AlphaTime or 0)+0.3 < CurTime()) then -- Avoids the VGUI sending "SetGateAddress" everytime we open it!
			...
			if (self.VGUI.AddressSelect) then
				timer.Remove("_Lib.RefreshAddressList");
				timer.Create("_Lib.RefreshAddressList",0.25,1,
					function()
	...
	self.VGUI.NameLabel:SetText(Lib.Language.GetMessage("stargate_vgui_name"));

	-- Name TextEntry
	self.VGUI.NameTextEntry:SetPos(75,pos[1]);
	self.VGUI.NameTextEntry:SetWide(110);
	self.VGUI.NameTextEntry:SetTooltip(Lib.Language.GetMessage("stargate_vgui_nametip"));
	...
	self.VGUI.NameTextEntry.OnTextChanged = function(TextEntry)
		...
		timer.Remove("_Lib.SetNameTime");
		timer.Create("_Lib.SetNameTime",1,1,
			function()
				if(IsValid(e)) then
					e:SetGateName(s);
				end
	...
	self.VGUI.PrivateCheckbox:SetText(Lib.Language.GetMessage("stargate_vgui_private"));
	self.VGUI.PrivateCheckbox:SetWide(110);
	local tip = Lib.Language.GetMessage("stargate_vgui_privatetip");
	...
	if (self.Class!="sg_supergate") then
		if (self.GroupSystem) then
			...
			self.VGUI.LocaleCheckbox:SetText(Lib.Language.GetMessage("stargate_vgui_locale"));
			local tip = Lib.Language.GetMessage("stargate_vgui_localetip");
		...
		elseif (self.Class!="sg_universe") then
			...
			self.VGUI.GalaxyCheckbox:SetText(Lib.Language.GetMessage("stargate_galaxy_vgui"));
			local tip = Lib.Language.GetMessage("stargate_galaxy_vgui_tip");
	...
	if (self.blocked_allowed) then
		...
		self.VGUI.BlockedCheckbox:SetText(Lib.Language.GetMessage("stargate_vgui_blocked"));
		local tip = Lib.Language.GetMessage("stargate_vgui_blockedtip");
	...
	if (not self.Alternative) then
		...
		for i=1,2 do
			local mul = (i-1);
			self.VGUI.AddressSelectLabel[i]:SetText(Lib.Language.GetMessage("stargate_vgui_dial"));
...
function PANEL:RegisterHooks()
	...
	self.Logo:SetImage("img/sg_logo");
...
vgui.Register("SControlerPanel",PANEL,"DFrame");
...
function PANEL:SetSettings(entity,groupsystem,candialg,nox,orlin)
	...
	self:SetCookieName("Lib.SControlerPanelDHD");
	...
	self.VGUI = {
		TitleLabel = vgui.Create("DLabel",self),
		AddressSelect = vgui.Create("SAddresseSelect",self),
		AddressSelectLabel = {
			vgui.Create("DLabel",self),
			vgui.Create("DLabel",self), -- Black background/Shadow
		},
	...
	if (self.Orlin) then
		...
		-- Group
		self.VGUI.GroupLabel:SetText(Lib.Language.GetMessage("stargate_vgui_group"));
		...
		self.VGUI.GroupTextEntry:SetTooltip(Lib.Language.GetMessage("stargate_vgui_grptip"));
		...
		self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp1"),"M@");
		self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp2"),"P@");
		self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp3"),"I@");
		self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp8"),"OT");
		self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp4"),"O@");
...
function PANEL:SetStatus(g,no_message)
	...
	if(g:len() == 2) then
		...
		if (g == "M@") then
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp1"));
		elseif (g == "P@") then
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp2"));
		elseif (g == "I@") then
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp3"));
		elseif (g == "OT") then
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp8"));
		elseif (g == "O@") then
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp4"));
		elseif (SG_CUSTOM_GROUPS and SG_CUSTOM_GROUPS[g]) then
			self.VGUI.GroupStatus:SetText(SG_CUSTOM_GROUPS[g][1]);
		else
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grpc"));
		end
	else
		-- Typing address!
		...
		elseif (g:len() == 2) then
			...
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_edit"));
		end
	end
end

vgui.Register("SControlerPanelDHD",PANEL,"DFrame");
...
function PANEL:SetSettings(entity,groupsystem)
	...
	self.VGUI.AddressLabel:SetText(Lib.Language.GetMessage("stargate_vgui_address"));
	self.VGUI.StatusLabel:SetText("");
	self.VGUI.MessageLabel:SetText("");
	self.Entity = entity;
	self.Class = entity:GetClass();
	self.GroupSystem = groupsystem;
	self.IsSuper = (self.Class=="sg_supergate");
	self.IsSGU = (self.Class=="sg_universe");
	...
	if (self.GroupAllowed) then
		...
		if (self.IsSGU) then
			self.VGUI.GroupLabel:SetText(Lib.Language.GetMessage("stargate_vgui_type"));		
		else
			self.VGUI.GroupLabel:SetText(Lib.Language.GetMessage("stargate_vgui_group"));
		end
		self.VGUI.GroupStatus:SetText("");
	...
	if (self.GroupSystem or self.IsSuper) then
		self.VGUI.AddressTextEntry:SetTooltip(Lib.Language.GetMessage("stargate_vgui_adrtip"));
	else
		self.VGUI.AddressTextEntry:SetTooltip(Lib.Language.GetMessage("stargate_galaxy_vgui_adrtip"));
	end
	--This function restricts the letters you can enter to a valid address
	if (self.GroupAllowed) then
		if (self.IsSGU) then
	...
	if (self.GroupAllowed) then
		...
		if (self.IsSGU) then
			...
			self.VGUI.GroupTextEntry.TextEntry:SetTooltip(Lib.Language.GetMessage("stargate_vgui_typetip"));
			--This function restricts the letters you can enter to a valid address
			self.VGUI.GroupTextEntry.TextEntry.OnTextChanged = function(TextEntry)
				local text = TextEntry:GetValue();
			...
			self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp5"),"U@#");
			self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp6"),"SGI");
			self.VGUI.GroupTextEntry:AddChoice(Lib.Language.GetMessage("stargate_vgui_grp7"),"DST");
			...
			self.VGUI.GroupTextEntry:SetPos(45,30);
			self.VGUI.GroupTextEntry:SetSize(35,self.VGUI.GroupTextEntry:GetTall());
			self.VGUI.GroupTextEntry.TextEntry:SetTooltip(Lib.Language.GetMessage("stargate_vgui_grptip"));
			--This function restricts the letters you can enter to a valid address
			self.VGUI.GroupTextEntry.TextEntry.OnTextChanged = function(TextEntry)
				local text = TextEntry:GetValue();
				local address = self.VGUI.AddressTextEntry:GetValue();
...
function PANEL:GetGates()
	...
	for _,v in pairs(StarGate_GetAll or {}) do
		if (self.IsSuper and v.class=="sg_supergate" or not self.IsSuper and v.groupgate) then
...
function PANEL:SetStatus(s,no_message)
	...
	if(len == 6) then
		...
		if(valid) then
			self.VGUI.StatusImage:SetImage(self.Images.Valid);
			self.VGUI.StatusLabel:SetText(Lib.Language.GetMessage("stargate_vgui_valid"));
			if(not no_message) then
				-- Have we set the address or is it the old from before?
				if(set) then
					self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid2"),"Info");
				else
					self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid3",s),"Info",true);
				end
			end
			...
			self.VGUI.StatusLabel:SetText(Lib.Language.GetMessage("stargate_vgui_invalid"));
			self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_adrexs"),"Warning");
		end
	elseif(len == 0) then
		-- Cleared this gate's address
		self.VGUI.StatusImage:SetImage("null");
		self.VGUI.StatusLabel:SetText("");
		if(not no_message) then self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_adrclr"),"Info") end;
		if(self.OnAddressSet) then self.OnAddressSet(self.Entity,"") end; -- CLEAR ADDRESS
	else
		-- Typing address!
		self.VGUI.StatusImage:SetImage(self.Images.Editing);
		self.VGUI.StatusLabel:SetText(Lib.Language.GetMessage("stargate_vgui_edit"));
		self:ShowMessage();
	end
...
function PANEL:SetStatusGroup(s,g,no_message,letters,gs)
	...
	if(len == 6 and g:len() == 2) then
		...
		if (valid) then
			for _,v in pairs(self:GetGates()) do
				...
				if(	address:find(letters[1]) and
					...
					if (self.Entity:GetNetworkedInt("SG_ATL_OVERRIDE")<=0 or (v.class=="sg_atlantis" and self.Entity:GetClass()=="sg_atlantis" or v.class!="sg_atlantis" and self.Entity:GetClass()!="sg_atlantis") or g:sub(1,1) == group:sub(1,1) and g:sub(2,2) != group:sub(2,2)) then
						if(tonumber(v.ent) == self.Entity:EntIndex()) then -- We have entered the same/similar address we had before - So we haven't changed anything
		...
		if(valid) then
			...
			if(not no_message) then
				...
				if(set) then
					if (IsValid(self.Entity) and self.Entity.GetGateGroup and g != self.Entity:GetGateGroup()) then
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid4"),"Info");
					else
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid2"),"Info");
					end
				else
					if (gs) then
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid4b",g),"Info");
					else
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid3",s),"Info",true);
					end
				end
			...
			self.VGUI.StatusImage:SetImage(self.Images.Invalid);
			self.VGUI.StatusLabel:SetText(Lib.Language.GetMessage("stargate_vgui_invalid"));
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_invalid"));
			if (l!="" and l:len()==1) then
				self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_symused",l),"Warning");
			elseif (l!="" and l:len()==2) then
				self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_symsused",l),"Warning");
			else
				self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_adrexs"),"Warning");
			end
		end
	...
	elseif(len == 0 and g:len() == 2 and g != self.Entity:GetGateGroup()) then
		...
		self.VGUI.StatusImage:SetImage("null");
		self.VGUI.StatusLabel:SetText("");
		if(not no_message) then self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid4"),"Info"); end;
		if(self.OnAddressSet) then self.OnAddressSet(self.Entity,"",g:upper()) end; -- CLEAR ADDRESS
		if (g == "M@") then
			self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp1"));
		elseif (g == "P@") then
...
function PANEL:SetStatusSGU(s,g,no_message,letters,gs)
	...
	if(len == 6 and g:len() == 3) then
		...
		if(valid) then
			...
			if(not no_message) then
				...
				if(set) then
					if (IsValid(self.Entity) and self.Entity.GetGateGroup and g != self.Entity:GetGateGroup()) then
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid5"),"Info");
					else
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid2"),"Info");
					end
				else
					if (gs) then
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid5b",g),"Info");
					else
						self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid3",s),"Info",true);
					end
				end
			...
			if(self.OnAddressSet) then self.OnAddressSet(self.Entity,s:upper(),g:upper()) end; -- SET THE ADDRESS!
			if (g == "U@#") then
				self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp5"));
			elseif (g == "SGI") then
				self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp6"));
			elseif (g == "DST") then
				self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp7"));
			elseif (SG_CUSTOM_TYPES and SG_CUSTOM_TYPES[g]) then
				self.VGUI.GroupStatus:SetText(SG_CUSTOM_TYPES[g][1]);
			else
				self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grpc2"));
			end
		else
			-- INVALID!
			self.VGUI.StatusImage:SetImage(self.Images.Invalid);
			...
			elseif (l!="" and l:len()>=2) then
				...
				if (valid_type) then
					...
					self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_typexs"),"Warning");
				end
			end
		end
	...
	elseif(len == 0 and g:len() == 3 and g != self.Entity:GetGateGroup()) then
		...
		if(valid) then
			...
			self.VGUI.StatusImage:SetImage("null");
			self.VGUI.StatusLabel:SetText("");
			if(not no_message) then self:ShowMessage(Lib.Language.GetMessage("stargate_vgui_valid5"),"Info"); end;
			if(self.OnAddressSet) then self.OnAddressSet(self.Entity,"",g:upper()) end; -- CLEAR ADDRESS
			if (g == "U@#") then
				self.VGUI.GroupStatus:SetText(Lib.Language.GetMessage("stargate_vgui_grp5"));
			elseif (g == "SGI") then
...
vgui.Register("SAddressePanel",PANEL,"Panel");
...
local function ApplyGlyphs(self, galaxy, small)
	...
	if (IsValid(ent) and ent:GetClass()=="sg_universe") then
		self.VGUI.AddressListView:SetDataHeight(hu);
		universe = true;
	elseif (IsValid(ent) and ent:GetClass()=="sg_atlantis") then
...
function PANEL:SetSettings(entity,groupsystem,candialg,hidedialmode)
	...
	self:SetCookieName("Lib.SAddresseSelect"); -- Just for the checkbox to save the value (it always annoyed me, it losses it after a restart)
	...
	if (self.GroupAllowed) then
		if (self.LocalGate) then
			...
			self.MaxSymbols = 6;
			self.VguiPos = {15,132,127};
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_glyphs")):SetFixedWidth(128);
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_address2")):SetFixedWidth(60);
			self.VGUI.Width = 60;
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_grouptype")):SetFixedWidth(100);
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_name2"));
		else
			self.AddressColumn = 2;
			self.ChoiceSize = 80;
			self.AllowedSymbols = "[^0-9A-Z@#]";
			...
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_glyphs")):SetFixedWidth(150);
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_address2")):SetFixedWidth(80);
			self.VGUI.Width = 80;
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_grouptype")):SetFixedWidth(100);
			self.VGUI.AddressListView:AddColumn(Lib.Language.GetMessage("stargate_vgui_name2"));
		end
	...
	--local w = 40
	--if (Lib.Language.ValidMessage("stargate_vgui_search_width")) then w = Lib.Language.GetMessage("stargate_vgui_search_width") end

	-- The Search Label
	self.VGUI.SearchLabel:SetPos(0,0);
	self.VGUI.SearchLabel:SetSize(40,self.VGUI.SearchLabel:GetTall());
	self.VGUI.SearchLabel:SetText(Lib.Language.GetMessage("stargate_vgui_search"));
	...
	self.VGUI.SearchTextEntry:SetTooltip(Lib.Language.GetMessage("stargate_vgui_srchtip"));
	...
	self.VGUI.SearchImageButton:SetTooltip(Lib.Language.GetMessage("stargate_vgui_search2"));
	...
	self.VGUI.RefreshImageButton:SetTooltip(Lib.Language.GetMessage("stargate_vgui_refresh"));
	...
	self.VGUI.AddressMultiChoice.TextEntry:SetTooltip(Lib.Language.GetMessage(self.AllowedSymbolsTxt));
	-- This function restricts the letters you can enter to a valid address
	if (self.GroupSystem or self.IsSuper) then
	...
	self.VGUI.DialTypeCheckbox:SetToolTip(Lib.Language.GetMessage("stargate_vgui_dialtip"));
	...
	self.VGUI.DialImageButton:SetToolTip(Lib.Language.GetMessage("stargate_vgui_dialgt"));
	...
	self.VGUI.AbortDialImageButton:SetToolTip(Lib.Language.GetMessage("stargate_vgui_dialab"));
...
function PANEL:GetGroupName(g)
	local name = Lib.Language.GetMessage("stargate_vgui_grpc")
	if (g == "M@") then
		name = Lib.Language.GetMessage("stargate_vgui_grp1");
	elseif (g == "P@") then
		name = Lib.Language.GetMessage("stargate_vgui_grp2");
	elseif (g == "I@") then
		name = Lib.Language.GetMessage("stargate_vgui_grp3");
	elseif (g == "OT") then
		name = Lib.Language.GetMessage("stargate_vgui_grp8");
	elseif (g == "O@") then
		name = Lib.Language.GetMessage("stargate_vgui_grp4");
	elseif (g == "U@#") then
		name = Lib.Language.GetMessage("stargate_vgui_grp5");
	elseif (g == "SGI") then
		name = Lib.Language.GetMessage("stargate_vgui_grp6");
	elseif (g == "DST") then
		name = Lib.Language.GetMessage("stargate_vgui_grp7");
	elseif (SG_CUSTOM_GROUPS and SG_CUSTOM_GROUPS[g]) then
		name = SG_CUSTOM_GROUPS[g][1];
	elseif (SG_CUSTOM_TYPES and SG_CUSTOM_TYPES[g]) then
		name = SG_CUSTOM_TYPES[g][1];
	elseif (g:len()==3) then
		name = Lib.Language.GetMessage("stargate_vgui_grpc2");
	end
	return name;
end
	...
	if (self.GroupAllowed) then
		...
		for k,v in pairs(self.Gates) do
			...
			if(tonumber(v.ent) ~= self.Entity:EntIndex() and not v.private) then
				...
				if(address != "" and group != "" and IsValid(ent) and (not locale and not ent:GetLocale() and not self.LocalGate or (ent:GetGateGroup() == group or v.class=="sg_universe" and ent:GetClass()=="sg_universe")) and (address!=ent:GetGateAddress() or group!=ent:GetGateGroup())) then
					...
					if (ent:GetGateGroup() != group and (v.class!="sg_universe" or ent:GetClass()!="sg_universe") or c_range > 0 and range>c_range and ent:GetGateGroup():len()==3) then
				...
				if(address ~= "") then
					if(IsValid(g))then
					    ...
					    if(v.galaxy or g:GetGalaxy() or
						   v.class=="sg_universe" and g:GetClass()=="sg_universe" and
						   c_range > 0 and range>c_range)then
						    address = address.."@";
					    end
					    if(v.class == "sg_atlantis" and g:GetClass() == "sg_atlantis" and #address == 7 and g:GetGalaxy() and v.galaxy)then
							address = string.Explode("@",tostring(address));
							address = address[1];
						end
						if(#address == 7 and g:GetGalaxy() and v.galaxy and ((v.class ~= "sg_atlantis" and g:GetClass() ~= "sg_atlantis") and
						   (v.class ~= "sg_universe" and g:GetClass() ~= "sg_universe")))then
							address = string.Explode("@",tostring(address));
							address = address[1];
						end
						if((v.class == "sg_universe" and g:GetClass() ~= "sg_universe") or --#address == 7 and
						   (v.class ~= "sg_universe" and g:GetClass() == "sg_universe"))then
							address = string.Explode("@",tostring(address));
							address = address[1].."@!";
						end
...
function PANEL:FindDHD()
	...
	local dhd = {};
	local DHDRange = Lib.CFG:Get("dhd","range",1000);
...
vgui.Register("SAddresseSelect",PANEL,"Panel");