...
MsgN("eap_librairies/vgui/init.lua")

-- Stargate Custom Groups sync with client @AlexALX
SG_CUSTOM_GROUPS = {}
SG_CUSTOM_TYPES = {}

net.Receive("_EAPCUSTOM_GROUPS",function(len)
...
vgui.Register("SHelperButton",PANEL,"Panel");
...
local PANEL = {};
PANEL.Data = {
	URL="http://sg-eap.space/wiki/", -- Base URL
	Width=ScrW()-100,
	Heigth=ScrH()-100,
}
...
function PANEL:Init()
	...
	self.Logo:SetImage("img/eap_large_logo");
...
vgui.Register("SHTMLHelper",PANEL,"DFrame");
...
local HIGHLIGHT = Color(50,50,50);
...
function PANEL:Init()
	...
	self.Key:SetFont("DermaDefaultBold");
	self.Key:SetText("");
...
function PANEL:OnMousePressed(mc)
	if(self.Edit) then return end; -- Must be disabled on edit. Otherwise, OnMousePressed overides our Lib.KeyBoard:GetPressedKey() function so we dont get "he pressed MOUSE1" etc
...
function PANEL:OnDoubleClick()
	--Lib.KeyBoard:GetPressedKey(); -- Most likely, you are holding down the spawnmenu or contextmenu key. This key is ignored once, except you press it again, if we run this once here
	self.Edit = true;
...
function PANEL:Think()
	...
	local key = input.CheckKeyTrapping(); --Lib.KeyBoard:GetPressedKey(); -- Currently pressed key
...
hook.Add("Initialize","SetKeyboardKey.Initialize",OnInitialize);

vgui.Register("SetKeyboardKey",PANEL,"Panel");
...
vgui.Register( "DProperty_EapNumber", PANEL, "DProperty_Generic" )